    ...
    if referee:
        ...
        with cache.lock:
            ...
            with sqlite3.connect(cache.db_path) as conn:
                ...
                if domain:
                    cursor.execute(
                        'SELECT email, name FROM referees WHERE email LIKE ?',
                        (f'%@{domain}',)
                    )
...
def clear_command(args):
    ...
    if args.journal:
        ...
        print(f"🧹 Cleared {deleted_manuscripts} manuscripts and {deleted_runs} runs older than {args.days} days")
                    ...
                    try:
                        ...
                        if stats:
                            ...
                            cache.update_journal_statistics(
                                ...
                                desk_rejection_rate=(stats[2] or 0.0) / 100.0  # Convert to 0-1 scale
                            )
                            stats_added += 1
                            print(f"   ✅ {journal}: {stats[0]} submissions, {stats[1]:.1f}% acceptance")
...
Examples:
  ...
  python cache_cli.py populate-stats
    ...
    clear_parser.add_argument('--days', type=int, default=90, help='Clear entries older than N days')
    ...
    populate_parser = subparsers.add_parser('populate-stats', help='Populate v1.0 compliance statistics')
