...
class SecureCredentialManager:
    ...
    def load_credentials(self):
        ...
        try:
            ...
            result_password = subprocess.run(
                cmd_password, capture_output=True, text=True, check=True
            )
    ...
    def delete_credentials(self):
        ...
        try:
            # Delete email
            subprocess.run(
                ["security", "delete-generic-password", "-s", f"{self.service_name}-email"],
                capture_output=True,
            )

            # Delete password
            subprocess.run(
                ["security", "delete-generic-password", "-s", f"{self.service_name}-password"],
                capture_output=True,
            )
