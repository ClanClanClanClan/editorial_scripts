...
class BrowserConfig:
    ...
    # Firefox preferences
    firefox_prefs: dict[str, any] = field(
        default_factory=lambda: {
    ...
    def get_chrome_options(self) -> list[str]:
        ...
        if self.disable_images:
            options.extend(["--blink-settings=imagesEnabled=false", "--disable-plugins"])
    ...
    def for_debugging(cls, **kwargs) -> "BrowserConfig":
        ...
        config.chrome_options.extend(["--remote-debugging-port=9222", "--enable-logging", "--v=1"])
        return config
