...
class ExtractionContract:
    ...
    def __init__(
        self, journal_code: str, journal_name: str, minimum_quality_threshold: float = 0.7
    ):
        """
    ...
    def _has_complete_manuscript_data(self, manuscript: Manuscript) -> bool:
        ...
        has_referees = (
            hasattr(manuscript, "referees") and manuscript.referees and len(manuscript.referees) > 0
        )
    ...
    def _calculate_quality_score(self, result: ExtractionResult) -> QualityScore:
        ...
        # PDF success rate
        if metrics.total_manuscripts_found > 0:
            score.pdf_success_rate = metrics.manuscripts_with_pdfs / metrics.total_manuscripts_found
        ...
        score.has_referee_reports = any(
            hasattr(m, "referees")
            and m.referees
            and any(hasattr(r, "report") and r.report for r in m.referees)
            for m in result.manuscripts
        )
        score.has_complete_dates = any(
            hasattr(m, "referees")
            and m.referees
            and any(hasattr(r, "dates") and r.dates for r in m.referees)
            for m in result.manuscripts
        )
    ...
    def validate(self, result: ExtractionResult) -> ValidationResult:
        ...
        """
        return self.validator.validate_extraction_result(result, self.minimum_quality_threshold)
    ...
    def get_requirements(self) -> dict[str, Any]:
        ...
        return {
            ...
            "required_data": {
                ...
                "pdfs": {"required": False, "minimum_count": 0, "preferred": True},
            },
