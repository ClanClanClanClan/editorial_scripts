...
class TestNotificationConfig:
    def test_get_default(self, client, tmp_path):
        with patch(
            "scripts.dashboard_server._NOTIFICATION_CONFIG_PATH",
            tmp_path / "nonexistent.json",
        ):
            resp = client.get("/api/notification-config")
        assert resp.status_code == 200
        data = resp.get_json()
        assert "email_enabled" in data or isinstance(data, dict)
...
class TestRefereeNote:
    def test_get_empty(self, client, tmp_path):
        with patch(
            "scripts.dashboard_server._REFEREE_NOTES_PATH",
            tmp_path / "nonexistent.json",
        ):
