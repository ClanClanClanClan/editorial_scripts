...
class SICON(SIAMJournalExtractor):
    """SICON journal extractor with all production features"""
    
    def __init__(self):
        """Initialize SICON extractor"""
        super().__init__('SICON')
        self.logger.info("Initialized SICON extractor with enhanced features")
    
    def post_process(self, manuscripts: List[dict]) -> List[dict]:
        """SICON-specific post-processing"""
        # Add any SICON-specific logic here
        for manuscript in manuscripts:
            ...
            # Add SICON-specific metadata
            manuscript['journal'] = 'SICON'
            manuscript['journal_full_name'] = 'SIAM Journal on Control and Optimization'
    ...
    def _check_needs_attention(self, manuscript: dict) -> bool:
        ...
        if manuscript.get('days_in_system', 0) > 180:
            return True
...
def run_sicon_extraction():
    """Convenience function to run SICON extraction"""
    extractor = SICON()
