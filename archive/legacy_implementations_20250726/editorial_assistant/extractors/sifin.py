"""SIAM Journal on Financial Mathematics (SIFIN) extractor."""
...
class SIFINExtractor(SIAMExtractor):
    """SIAM Journal on Financial Mathematics extractor using ORCID authentication."""

    def __init__(self, journal: JournalConfig):
        super().__init__(journal)
        self.site_prefix = "https://sifin.siam.org/"

    def _login(self) -> None:
        """Login to SIFIN using ORCID authentication."""
        try:
            # Navigate to SIFIN URL
            ...
            orcid_login_link = self._wait_for_element_by_xpath(
                "//a[contains(@href, 'orcid') or contains(text(), 'ORCID')]", timeout=10
            )
            ...
            if not orcid_login_link:
                raise Exception("No ORCID login link found on SIFIN page")
            ...
            self._wait_for_url_contains("sifin.siam.org", timeout=30)
    ...
    def extract_manuscripts(self) -> list[dict[str, Any]]:
        """Extract manuscripts from SIFIN dashboard."""
        try:
        ...
        for assoc_ed_section in soup.find_all("tbody", {"role": "assoc_ed"}):
            for row in assoc_ed_section.find_all("tr", class_="ndt_task"):
                ...
                if "form_type=view_ms" in href:
                    full_url = (
                        href if href.startswith("http") else self.site_prefix + href.lstrip("/")
                    )
        ...
        for row in table.find_all("tr"):
            ...
            if label.startswith("Manuscript #"):
                ...
                for ref_link in td.find_all("a"):
                    ...
                    abs_url = (
                        referee_url
                        if referee_url.startswith("http")
                        else self.site_prefix + referee_url.lstrip("/")
                    )
    ...
    def _navigate_to_manuscripts(self) -> None:
        ...
        try:
            # For SIFIN, we navigate to the main dashboard
            self.driver.get(self.journal.url)
            self._remove_cookie_banner()
    ...
    def _process_manuscript(self, manuscript) -> None:
        ...
        try:
            ...
            if not matching_url:
                logging.warning(
                    f"[{self.journal.code}] No URL found for manuscript {manuscript.manuscript_id}"
                )
                return
            ...
            if manuscript_data:
                ...
                for ref_data in manuscript_data.get("Referees", []):
                    referee = Referee(
                        ...
                        status=RefereeStatus.CONTACTED
                        if ref_data.get("Status") == "Contacted"
                        else RefereeStatus.ACCEPTED,
            ...
            logging.error(
                f"[{self.journal.code}] Error processing manuscript {manuscript.manuscript_id}: {e}"
            )
