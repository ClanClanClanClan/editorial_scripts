...
from src.core.data_models import Manuscript, Referee, Document, DocumentType, RefereeStatus
...
class ScholarOneExtractor(BaseExtractor):
    ...
    def _handle_2fa(self, login_click_time: datetime) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"Using gmail_verification_wrapper for 2FA (codes after {login_click_time})")
            ...
            if not code:
                try:
                    ...
                    gmail = GmailManager()
                    if gmail.service:
                        self.logger.info(f"Gmail service connected, waiting up to 120s for verification email after {login_click_time}")
                        ...
                        if code:
                            self.logger.info(f"✅ Successfully retrieved 2FA code via Gmail API")
                        else:
                    ...
                    self.logger.debug(f"Will fall back to manual entry")
            ...
            try:
                error_modal = self.driver.find_element(By.XPATH, "//h3[contains(text(),'Verification Error')]")
                if error_modal.is_displayed():
                    self.logger.warning("Verification error detected - code might be incorrect or expired")
            ...
            try:
                success_modal = self.driver.find_element(By.XPATH, "//h3[contains(text(),'Success')]")
                if success_modal.is_displayed():
                    self.logger.info("Success modal detected - closing it")
                    try:
                        close_btn = self.driver.find_element(By.XPATH, "//button[contains(text(),'Close') or contains(@class,'close')]")
                        close_btn.click()
                        time.sleep(2)
                    except:
                        pass
    ...
    def _is_manuscript_category(self, name: str) -> bool:
        ...
        keywords = [
            'manuscript', 'review', 'decision', 'revision',
            'awaiting', 'pending', 'submitted', 'score',
            'overdue', 'selection', 'invitation', 'assignment',
            'recommendation'
        ]
        return any(keyword in name.lower() for keyword in keywords)
    
    def extract_manuscript_id(self, text: str) -> Optional[str]:
    ...
    def extract_document_links(self) -> List[Document]:
        ...
        try:
            ...
            supp_links = self.driver.find_elements(
                By.XPATH, "//a[contains(text(),'Supplement')]"
            )
            for link in supp_links:
                doc = Document(
                    type=DocumentType.SUPPLEMENTARY,
                    url=link.get_attribute('href')
                )
                documents.append(doc)
    ...
    def navigate_next_document(self) -> bool:
        ...
        try:
            next_btn = self.driver.find_element(
                By.XPATH, "//a[contains(@href,'XIK_NEXT_PREV_DOCUMENT_ID')]/img[@alt='Next Document']/.."
            )
    ...
    def navigate_previous_document(self) -> bool:
        ...
        try:
            prev_btn = self.driver.find_element(
                By.XPATH, "//a[contains(@href,'XIK_NEXT_PREV_DOCUMENT_ID')]/img[@alt='Previous Document']/.."
            )
