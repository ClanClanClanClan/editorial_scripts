...
from .redis_cache import APIResponseCache, create_redis_layer
...
class MultiLayerCache:
    ...
    def __init__(self, cache_dir: Path = None, test_mode: bool = None, redis_url: str = None):
        ...
        if self.redis_cache and self.redis_cache.enabled:
            print("   🌐 Layer 4: Redis (active)")
        else:
            print("   🌐 Layer 4: Redis (not available)")
    ...
    def _get_from_filesystem(self, cache_key: str) -> Any | None:
    ...
    def get(self, cache_type: str, key_components: dict[str, Any]) -> Any | None:
        """
        ...
        """
        cache_key = self._compute_cache_key({"type": cache_type, **key_components})
    ...
    def set(
        self, cache_type: str, key_components: dict[str, Any], data: Any, ttl_seconds: int = 3600
    ):
        """
        ...
        if cache_type == "referee" and hasattr(data, "email"):
            ...
            if domain and isinstance(data, dict):
                self.sqlite_cache.cache_institution(
                    domain, data.get("institution", ""), data.get("country", "")
                )
        ...
        if cache_type == "api_response" and self.redis_cache and self.redis_cache.enabled:
            ...
            and cache_type in ["api_response", "shared_data", "referee_search"]
        ):
    ...
    def get_cache_stats(self) -> dict[str, Any]:
        ...
        memory_size = sum(len(json.dumps(item["data"])) for item in self.memory_cache.values())
        ...
        return {
            ...
            "layer_4_redis": self.redis_cache.get_stats()
            if self.redis_cache and self.redis_cache.enabled
...
class EnhancedExtractorCacheMixin:
    ...
    def get_cached_data(self, data_type: str, **key_components) -> Any | None:
    ...
    def finish_enhanced_extraction(self, metadata: dict[str, Any] = None):
        ...
        self.multi_cache.sqlite_cache.update_extraction_stats(self.run_id, self.extraction_stats)
        self.multi_cache.sqlite_cache.finish_extraction_run(self.run_id, enhanced_metadata)
        ...
        if redis_stats.get("status") == "enabled":
            print(
                f"   🌐 Layer 4 (Redis): {redis_stats.get('keys_count', 0)} keys, {redis_stats.get('memory_used', 'unknown')}"
            )
        else:
