...
def display_referee_data(referee):
    ...
    if referee.get('orcid'):
        ...
        if referee.get('orcid_discovered'):
            print(f"            → Discovered via ORCID API (confidence: {referee.get('orcid_confidence', 0):.0%})")
    ...
    if referee.get('affiliation_history'):
        ...
        if current:
            curr = current[0]
            print(f"         🏛️ Current position: {curr.get('role', 'Unknown')} at {curr.get('organization', 'Unknown')}")
...
def display_author_data(author):
    ...
    if author.get('orcid'):
        ...
        if author.get('orcid_discovered'):
            print(f"            → Discovered via ORCID API (confidence: {author.get('orcid_confidence', 0):.0%})")
    ...
    try:
        ...
        if extractor.manuscripts:
            ...
            referees_with_orcid = sum(
                1 for ms in extractor.manuscripts
