...
def generate_timeline_report():
        ...
        for ms in sorted(current_manuscripts, key=lambda x: x["id"]):
            ...
            for ref in referees:
                ...
                if ref.get("report_submitted"):
                    report.append(
                        f"    ✅ Report submitted: {ref.get('report_date', 'Unknown')[:10]}"
                    )
                elif ref.get("response") == "Accepted":
                    report.append("    ⏳ Awaiting report")
            ...
            if timeline:
                ...
                # Find key events
                submission = next(
                    (e for e in timeline if "submission" in e.get("type", "").lower()), None
                )
                if submission:
                ...
                if acceptances:
                    for acc in acceptances:
                        report.append(
                            f"  • {acc['details']['referee_accepted']} accepted: {acc['date'][:10]}"
                        )
                ...
                if declines:
                    for dec in declines:
                        report.append(
                            f"  • {dec['details']['referee_declined']} declined: {dec['date'][:10]}"
                        )

                report_submissions = [
                    e for e in timeline if e.get("details", {}).get("report_submitted_by")
                ]
                if report_submissions:
                    for sub in report_submissions:
                        report.append(
                            f"  • Report from {sub['details']['report_submitted_by']}: {sub['date'][:10]}"
                        )
    ...
    if historical_manuscripts:
        ...
        for ms in sorted(historical_manuscripts, key=lambda x: x["id"], reverse=True):
            ...
            report.append(f"📄 {ms['id']}: {ms['title'][:60]}...")
            report.append(
                f"   Status: {ms['status']} | Referees: {len(ms.get('referees', []))} | Reports: {len(ms.get('referee_reports', []))}"
            )
    ...
    total_accepted = sum(
        sum(1 for r in m.get("referees", []) if r.get("response") == "Accepted")
        for m in manuscripts
    )
    total_declined = sum(
        sum(1 for r in m.get("referees", []) if r.get("response") == "Declined")
        for m in manuscripts
    )
    ...
    if referee_counts:
        ...
        for name, stats in sorted(
            referee_counts.items(), key=lambda x: x[1]["total"], reverse=True
        ):
            report.append(
                f"  • {name}: {stats['total']} assignments, {stats['accepted']} accepted, {stats['reports']} reports"
            )
