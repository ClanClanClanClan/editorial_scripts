...
from base.optimized_base_extractor import OptimizedBaseExtractor
from ..base.optimized_base_extractor import ConnectionConfig
...
class OptimizedSICONExtractor(OptimizedBaseExtractor):
    ...
    async def _perform_authentication(self) -> bool:
        ...
        try:
            ...
            # Submit login
            submit_selectors = ['#signin-button', '[type="submit"]', 'button:contains("Sign in")', '.btn-primary']
        ...
        try:
            ...
            for category_name, category_url in ae_links.items():
                try:
                    ...
                    category_manuscripts = await self._extract_manuscripts_from_category(category_url)
    ...
    async def _parse_manuscript_metadata_optimized(self, soup: BeautifulSoup) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Parsed metadata: title='{metadata['title'][:50]}...', authors={len(metadata['authors'])}")
    ...
    async def _extract_referees_optimized(self, soup: BeautifulSoup, manuscript_id: str) -> List[OptimizedReferee]:
        """
