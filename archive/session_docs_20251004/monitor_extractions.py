...
class ExtractionMonitor:
    ...
    def calculate_health_metrics(self, days: int = 7) -> Dict:
        ...
        if recent_results:
            ...
            for result in recent_results:
                ...
                metrics["journal_breakdown"][journal]["manuscripts"] += result.get(
                    "manuscripts_count", 0
                )
    ...
    def show_dashboard(self, days: int = 7):
        ...
        if metrics["journal_breakdown"]:
            ...
            print(f"{'-' * 30}")
            print(
                f"{'Journal':<8} | {'Runs':<4} | {'Papers':<6} | {'Success':<7} | {'Avg Time':<8}"
            )
            print(f"{'-' * 50}")
            ...
            for journal, data in metrics["journal_breakdown"].items():
                ...
                print(
                    f"{journal.upper():<8} | {data['extractions']:<4} | {data['manuscripts']:<6} | "
    ...
    def check_health(self) -> Tuple[bool, List[str]]:
        ...
        if metrics["avg_duration_seconds"] > 600:
            issues.append(
                f"Extractions taking too long: {metrics['avg_duration_seconds']:.1f}s avg"
            )
...
def main():
    ...
    parser.add_argument("--dashboard", "-d", action="store_true", help="Show monitoring dashboard")
    ...
    parser.add_argument("--output", "-o", help="Output CSV file (for --export)")
    ...
    if args.dashboard:
        ...
        if results:
            ...
            print(f"\n📊 QUICK STATUS")
            print(
                f"Latest extraction: {latest.get('journal_id', 'unknown').upper()} "
