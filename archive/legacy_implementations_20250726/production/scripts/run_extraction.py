...
def _validate_against_baseline(journal_code, manuscripts):
    ...
    if journal_upper in baselines["journals"]["active"]:
        ...
        if "referees" in expected:
            ...
            manuscript_rate = (
                actual_manuscripts / expected["manuscripts"] if expected["manuscripts"] > 0 else 0
            )
            referee_rate = (
                actual_referees / expected["referees"]["total"]
                if expected["referees"]["total"] > 0
                else 0
            )
        ...
        print("   Status: Inactive journal (0 manuscripts expected)")
        if len(manuscripts) == 0:
            print("   ✅ Correctly shows no manuscripts")
        else:
...
def main():
    ...
    parser.add_argument(
        "journal",
        choices=["sicon", "sifin", "mf", "mor", "naco", "fs", "jota", "mafe"],
    ...
    journals = {
        "sicon": JournalConfig(
            ...
            platform="siam",
            credentials={"username_env": "ORCID_EMAIL", "password_env": "ORCID_PASSWORD"},
        ),
        "sifin": JournalConfig(
            code="SIFIN",
        ...
        "mf": JournalConfig(
            ...
            platform="scholarone",
            credentials={"username_env": "MF_EMAIL", "password_env": "MF_PASSWORD"},
        ),
        "mor": JournalConfig(
            code="MOR",
            ...
            platform="scholarone",
            credentials={"username_env": "MOR_EMAIL", "password_env": "MOR_PASSWORD"},
        ),
        "naco": JournalConfig(
            code="NACO",
        ...
        "fs": JournalConfig(
            ...
            credentials={"username_env": "FS_EMAIL", "password_env": "FS_PASSWORD"},
        ),
        "jota": JournalConfig(
            code="JOTA",
            ...
            platform="editorial_manager",
            credentials={"username_env": "JOTA_EMAIL", "password_env": "JOTA_PASSWORD"},
        ),
        "mafe": JournalConfig(
            code="MAFE",
            ...
            platform="editorial_manager",
            credentials={"username_env": "MAFE_EMAIL", "password_env": "MAFE_PASSWORD"},
        ),
    }
    ...
    try:
        ...
        for i, ms in enumerate(manuscripts[:3]):
            if hasattr(ms, "manuscript_id"):
                ...
                status = ms.get(
                    "current_stage", ms.get("Current Stage", ms.get("status", "Unknown"))
                )
