...
def analyze(ctx, input_file, conflicts, statistics, output):
    ...
    if output:
        ...
        console.print("[yellow]Output saving not yet implemented[/yellow]")
...
def _show_statistics(console, data):
    ...
    table.add_row(
        "Average Referees per MS", f"{total_referees/len(manuscripts):.1f}" if manuscripts else "0"
    )
        ...
        for r in m.get("referees", []):
            ...
            if institution:
                ...
                institution_referees[institution].append(
                    {"name": r.get("name"), "manuscript": m.get("manuscript_id")}
                )
