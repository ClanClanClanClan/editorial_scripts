...
def test_orcid_client():
    ...
    try:
        ...
        if enriched2.get('orcid_discovered'):
            print(f"    → Discovered via API (confidence: {enriched2.get('orcid_confidence', 0):.0%})")
...
def run_full_extraction():
    ...
    print("\n📋 COMPONENT STATUS:")
    print(f"  • ORCID Client: {'✅' if hasattr(extractor, 'orcid_client') and extractor.orcid_client else '❌'}")
...
def analyze_results(manuscripts):
    ...
    # ORCID Coverage
    authors_with_orcid = sum(1 for ms in manuscripts
                             for a in ms.get('authors', [])
                             if a.get('orcid'))
    authors_discovered = sum(1 for ms in manuscripts
                            for a in ms.get('authors', [])
                            if a.get('orcid_discovered'))

    referees_with_orcid = sum(1 for ms in manuscripts
                              for r in ms.get('referees', [])
                              if r.get('orcid'))
    referees_discovered = sum(1 for ms in manuscripts
                             for r in ms.get('referees', [])
                             if r.get('orcid_discovered'))

    print(f"\n🆔 ORCID COVERAGE:")
    print(f"  AUTHORS:")
    print(f"    • With ORCID: {authors_with_orcid}/{total_authors} ({authors_with_orcid/max(total_authors,1)*100:.0f}%)")
    print(f"    • From journal: {authors_with_orcid - authors_discovered}")
    print(f"    • Discovered via API: {authors_discovered}")
    print(f"  REFEREES:")
    print(f"    • With ORCID: {referees_with_orcid}/{total_referees} ({referees_with_orcid/max(total_referees,1)*100:.0f}%)")
    print(f"    • From journal: {referees_with_orcid - referees_discovered}")
    print(f"    • Discovered via API: {referees_discovered}")

    # Country Coverage
    authors_with_country = sum(1 for ms in manuscripts
                               for a in ms.get('authors', [])
                               if a.get('country'))
    referees_with_country = sum(1 for ms in manuscripts
                                for r in ms.get('referees', [])
                                if r.get('country'))

    print(f"\n🌍 COUNTRY DATA:")
    print(f"  • Authors with country: {authors_with_country}/{total_authors} ({authors_with_country/max(total_authors,1)*100:.0f}%)")
    print(f"  • Referees with country: {referees_with_country}/{total_referees} ({referees_with_country/max(total_referees,1)*100:.0f}%)")

    # Publication Data
    total_publications = sum(r.get('publication_count', 0)
                           for ms in manuscripts
                           for r in ms.get('referees', []))
    referees_with_pubs = sum(1 for ms in manuscripts
                             for r in ms.get('referees', [])
                             if r.get('publication_count', 0) > 0)
    ...
    # Research Interests
    referees_with_interests = sum(1 for ms in manuscripts
                                  for r in ms.get('referees', [])
