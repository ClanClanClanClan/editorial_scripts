...
class AuthorExtractionStep(ExtractionStep):
    ...
    def execute(self, context: ExtractionContext) -> ExtractionContext:
        ...
        try:
            ...
            author_rows = context.browser.driver.find_elements(
                By.XPATH,
                context.config.selectors.author_row
            )
    ...
    def _extract_author_from_row(self, row, context: ExtractionContext) -> Optional[Dict[str, Any]]:
...
class RefereeExtractionStep(ExtractionStep):
    ...
    def execute(self, context: ExtractionContext) -> ExtractionContext:
        ...
        try:
            ...
            referee_rows = context.browser.driver.find_elements(
                By.XPATH,
                context.config.selectors.referee_row
            )
    ...
    def _extract_referee_from_row(self, row, context: ExtractionContext) -> Optional[Dict[str, Any]]:
...
class ExtractionPipeline:
    ...
    def __init__(self, config: JournalConfig):
        ...
        self.browser: Optional[SeleniumBrowserManager] = None
        self.popup_handler: Optional[PopupHandler] = None
        self.authenticator: Optional[ScholarOneAuthenticator] = None
    ...
    def extract_manuscript(self, manuscript_id: str) -> ExtractedManuscript:
        ...
        # Log any errors
        if context.errors:
            self.logger.warning(f"Extraction completed with errors for {manuscript_id}: {context.errors}")
...
class PipelineBuilder:
    ...
    def add_navigation(self) -> 'PipelineBuilder':
        ...
        self.pipeline.add_step(
            NavigationStep(self.config.urls.manuscript_details_url)
        )
        return self
