"""
SIAM Journal on Control and Optimization (SICON) Scraper
...
class SICONScraper(JournalExtractor):
    """SICON journal scraper for SIAM ORCID platform"""
    ...
    def __init__(self, browser_pool: BrowserPool):
        ...
        self.folder_id = "1800"  # SICON folder ID
        
    async def extract_manuscripts(self) -> List[Manuscript]:
        """Extract manuscripts and referee data from SICON journal"""
        async with self.browser_pool.get_browser() as browser:
            ...
            try:
                ...
                self.logger.error(f"Error extracting SICON manuscripts: {e}")
                raise
            finally:
                await page.close()
    
    async def _login(self, page: Page) -> None:
        """Login to SICON journal using ORCID"""
        self.logger.info("Logging into SICON journal...")
        ...
        if not email or not password:
            raise ValueError("ORCID credentials not configured for SICON")
        ...
        self.logger.info("Successfully logged into SICON")
        ...
        try:
            ...
            for i, cell in enumerate(cells):
                ...
                if not manuscript_id and re.match(r'\d+', cell_text):
                    manuscript_id = f"SICON-{cell_text}"
            ...
            if not manuscript_id:
                manuscript_id = f"SICON-{hash(title) % 10000}"
    ...
    async def _parse_manuscript_div(self, div) -> Optional[Manuscript]:
        ...
        try:
            ...
            id_match = re.search(r'SICON[-_]?\d+|\d{4,}', div_text)
            manuscript_id = id_match.group(0) if id_match else f"SICON-{hash(title) % 10000}"
    ...
    def _map_manuscript_status(self, status_text: str) -> ManuscriptStatus:
        """Map SICON status to domain status"""
