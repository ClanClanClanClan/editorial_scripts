...
"""
Debug string length validation issue
"""
...
print("🧪 Testing String Length Validation")
print("=" * 50)

# Test maximum string lengths
extreme_data = generate_valid_metrics()
extreme_data["name"] = "X" * 200  # Max length
extreme_data["email"] = "a" * 180 + "@test.com"  # Long email
extreme_data["institution"] = "University " * 45  # Long institution

print(f"Name length: {len(extreme_data['name'])}")
print(f"Email length: {len(extreme_data['email'])}")
print(f"Institution length: {len(extreme_data['institution'])}")

try:
    response = client.post("/api/v1/referees/", json=extreme_data)
    print(f"Status: {response.status_code}")
    
    if response.status_code == 201:
        print(f"✅ Created with long strings")
    else:
