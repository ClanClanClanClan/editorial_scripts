...
class EmailVerificationManager:
    ...
    def _check_legacy_availability(self) -> bool:
        ...
        try:
            ...
            if legacy_email_utils.exists():
                session_manager.add_learning("Legacy email utilities available for 2FA verification")
                return True
            else:
                session_manager.add_learning("Legacy email utilities not found - 2FA verification may be limited")
                return False
    ...
    def fetch_verification_code(self, journal: str, max_wait: int = 120, poll_interval: int = 5) -> Optional[str]:
        """
        ...
        if self._legacy_available:
            ...
            self.logger.warning("Legacy email utilities not available - cannot fetch verification code")
            return None
    
    def _fetch_using_legacy(self, journal: str, max_wait: int, poll_interval: int) -> Optional[str]:
        """
        ...
        try:
            ...
            if verification_code:
                session_manager.add_learning(f"Successfully fetched verification code for {journal}: {verification_code}")
                return verification_code
            else:
                session_manager.add_learning(f"No verification code found for {journal} within {max_wait} seconds")
                return None
    ...
    def validate_email_setup(self) -> bool:
        ...
        try:
            ...
            if missing_vars:
                self.logger.warning(f"Missing environment variables for email verification: {missing_vars}")
                session_manager.add_learning(f"Email verification setup incomplete: missing {missing_vars}")
                return False
            ...
            if self._legacy_available:
                try:
                    ...
                    if service:
                        session_manager.add_learning("Email verification setup validated successfully")
                        return True
                    else:
