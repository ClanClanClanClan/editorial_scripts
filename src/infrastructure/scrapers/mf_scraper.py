"""
Mathematical Finance (MF) Journal Scraper
...
class MFScraper(JournalExtractor):
    ...
    async def extract_manuscripts(self) -> List[Manuscript]:
        """Extract manuscripts and referee data from MF journal"""
        async with self.browser_pool.get_browser() as browser:
            ...
            try:
                ...
                self.logger.error(f"Error extracting MF manuscripts: {e}")
                raise
            finally:
                await page.close()
    
    async def _login(self, page: Page) -> None:
        """Login to Mathematical Finance journal"""
        self.logger.info("Logging into MF journal...")
        ...
        # Enter credentials
        username = self.settings.mf_username or ""
        password = self.settings.mf_password or ""
        
        if not username or not password:
            raise ValueError("MF credentials not configured")
        ...
        self.logger.info("Successfully logged into MF")
    ...
    async def _handle_2fa(self, page: Page) -> None:
        ...
        try:
            ...
            if code_input:
                ...
                # For now, we'll wait for manual input or implement email fetching
    ...
    def _map_manuscript_status(self, status_text: str) -> ManuscriptStatus:
        """Map MF status to domain status"""
    ...
    def _map_referee_status(self, status_text: str) -> RefereeStatus:
        """Map MF referee status to domain status"""
        status = status_text.lower().strip()
