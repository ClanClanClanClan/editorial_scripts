...
class FSExtractor(EmailBasedExtractor):
    ...
    def extract_from_emails(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            email_subjects = self.journal.patterns.get(
    ...
    def _search_editor_communications(
        self, subjects: list[str], lookback_days: int
    ) -> list[dict[str, Any]]:
    ...
    def _parse_email_content(self, email: dict[str, Any]) -> dict[str, Any] | None:
    ...
    def _extract_manuscript_id(self, subject: str, body: str) -> str | None:
    ...
    def _parse_referee_email(
        self, manuscript_id: str, subject: str, body: str, date: datetime
    ) -> dict[str, Any]:
            ...
            if manuscript.get("referees"):
                for referee in manuscript["referees"]:
                    ...
                    for existing in merged[manuscript_id]["Referees"]:
                        if existing.get("Referee Name") == referee.get("name") or existing.get(
                            "Referee Email"
                        ) == referee.get("email"):
