...
def fetch_latest_verification_code(journal: str, max_wait: int = 300, poll_interval: int = 10, start_timestamp: float = None) -> str:
    """
    ...
    try:
        ...
        if start_timestamp:
            print(f"   📅 Looking for codes received after {time.strftime('%H:%M:%S', time.localtime(start_timestamp))}")
        ...
        while (time.time() - search_start_time) < max_wait:
            try:
                ...
                for message in messages:
                    ...
                    if any(term in full_text for term in ['verification', 'access code', 'authentication', 'manuscript', 'scholarone', journal.lower()]):
                ...
                for msg_data in verified_messages:
                    ...
                    if code:
                        ...
                        print(f"   ✅ Found FRESH verification code from email at {msg_time}: {code[:3]}***")
                        return code
