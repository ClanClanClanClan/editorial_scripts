...
class MAFEExtractor(EditorialManagerExtractor):
        ...
        try:
            ...
            for i, iframe in enumerate(iframes):
                try:
                    ...
                    if username_field and password_field:
                        logging.info(
                            f"[{self.journal.code}] Found login fields in iframe {i} with Playwright"
                        )
                        ...
                        if login_button:
                            login_button.click()
                            logging.info(
                                f"[{self.journal.code}] Clicked Editor Login with Playwright"
                            )
        ...
        try:
            ...
            for folder_text, nvgurl in folder_links:
                try:
                    ...
                    logging.error(
                        f"[{self.journal.code}] Error processing folder {folder_text}: {e}"
                    )
                    continue
    ...
    def _collect_folder_links(self) -> list[tuple]:
        ...
        try:
            ...
            logging.info(
                f"[{self.journal.code}] Found {len(folder_links)} folders with manuscripts"
            )
            return folder_links
    ...
    def _get_referee_email_selenium(self, link_element, referee_name: str) -> str:
        ...
        try:
            ...
            try:
                ...
                logging.warning(
                    f"[{self.journal.code}] Failed to get email for {referee_name}: {e}"
                )
            ...
            logging.warning(
                f"[{self.journal.code}] Email extraction failed for {referee_name}: {e}"
            )
            return ""
