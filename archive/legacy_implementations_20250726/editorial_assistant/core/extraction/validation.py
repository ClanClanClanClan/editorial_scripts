...
from .models import ExtractionResult, ExtractionStatus, QualityScore
...
class QualityValidator:
    ...
    def validate_extraction_result(self, result: ExtractionResult, 
                                 minimum_quality_threshold: float = None) -> ValidationResult:
        """
    ...
    def _validate_structure(self, result: ExtractionResult, 
    ...
    def _validate_manuscripts(self, result: ExtractionResult,
                            validation: ValidationResult) -> None:
    ...
    def _validate_referees(self, result: ExtractionResult,
                         validation: ValidationResult) -> None:
    ...
    def _validate_pdfs(self, result: ExtractionResult,
                      validation: ValidationResult) -> None:
    ...
    def _validate_quality_score(self, result: ExtractionResult,
                               validation: ValidationResult,
    ...
    def _validate_metrics(self, result: ExtractionResult,
                         validation: ValidationResult) -> None:
        ...
        # Check error rates
        total_operations = (
            metrics.total_manuscripts_processed + 
            metrics.total_pdfs_downloaded
        )
    ...
    def _validate_status(self, result: ExtractionResult,
                        validation: ValidationResult) -> None:
    ...
    def _generate_summary(self, result: ExtractionResult,
                         validation: ValidationResult) -> None:
    ...
    def _generate_recommendations(self, result: ExtractionResult,
                                validation: ValidationResult) -> None:
        """Generate improvement recommendations."""
        # Priority recommendations based on issues
        error_categories = {
            issue.category for issue in validation.issues 
        ...
        if 'manuscripts' in error_categories:
            validation.add_recommendation(
                "Improve manuscript detection and ID extraction patterns"
            )
        
        if 'referees' in error_categories:
            validation.add_recommendation(
                "Review referee extraction selectors and parsing logic"
            )
        ...
        if not result.quality_score.has_manuscript_pdfs:
            validation.add_recommendation(
                "Consider implementing PDF download functionality"
            )
