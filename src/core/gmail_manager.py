...
class GmailManager:
    ...
    def get_verification_code(
        ...
        self.logger.info(f"Waiting 5 seconds for email system to send verification email...")
        time.sleep(5)
        ...
        while datetime.now() < end_time:
            ...
            self.logger.debug(f"Attempt {attempt}: Checking for verification emails ({remaining_time}s remaining)")
            try:
                ...
                for message in messages:
                    ...
                    if date_str:
                        ...
                        if email_time > start_time:
                            ...
                            if code:
                                self.logger.info(f"Found verification code: {code[:3]}*** (sent at {email_time})")
                                return code
                        else:
    ...
    def _get_body_from_message(self, message: Dict[str, Any]) -> Optional[str]:
