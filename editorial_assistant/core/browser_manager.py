...
class BrowserManager:
    ...
    def create_driver(self) -> WebDriver:
        ...
        for i, strategy in enumerate(strategies):
            try:
                self.logger.info(f"Attempting driver creation strategy {i+1}/{len(strategies)}: {strategy.__name__}")
                driver = strategy()
                ...
                if i == len(strategies) - 1:
                    raise BrowserError(f"All driver creation strategies failed. Last error: {str(e)}")
    ...
    def handle_download(self, driver: WebDriver, timeout: int = 30) -> Optional[Path]:
        """
