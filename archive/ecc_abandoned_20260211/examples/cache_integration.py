...
from src.ecc.core.performance_cache import CacheStrategy, PerformanceCache, create_extraction_cache
...
class CachedExtractionExample:
    ...
    def simulate_popup_extraction(self, popup_url: str) -> str:
        ...
        self.logger.info(
            f"Extracting email from popup: {popup_url[:50]}...", category=LogCategory.POPUP
        )
        ...
        if cached_email:
            self.cache_hits += 1
            self.logger.success(
                f"Cache hit - Found email: {cached_email}", category=LogCategory.POPUP
            )
            return cached_email
        ...
        self.logger.warning("Cache miss - extracting from popup", category=LogCategory.POPUP)
        ...
        self.logger.success(
            f"Extracted and cached email: {extracted_email}", category=LogCategory.POPUP
        )
    ...
    def simulate_referee_lookup(self, referee_name: str) -> dict:
        ...
        self.logger.info(f"Looking up referee: {referee_name}", category=LogCategory.EXTRACTION)

        # Check cache first
        cached_data = self.cache.get_cached_referee(referee_name)
        if cached_data:
            self.cache_hits += 1
            self.logger.success("Cache hit - Found referee data", category=LogCategory.EXTRACTION)
            return cached_data
        ...
        self.logger.warning(
            "Cache miss - performing database lookup", category=LogCategory.EXTRACTION
        )
        ...
        self.logger.success("Referee data looked up and cached", category=LogCategory.EXTRACTION)
    ...
    def simulate_manuscript_processing(self, manuscript_id: str) -> dict:
        ...
        try:
            ...
            if cached_status:
                self.logger.success(
                    f"Using cached status: {cached_status}", category=LogCategory.DATA
                )
            else:
                ...
                self.logger.info(
                    f"Status looked up and cached: {cached_status}", category=LogCategory.DATA
                )
            ...
            popup_urls = [
                f"javascript:popWindow('{name.replace(' ', '')}')" for name in referee_names
            ]

            for name, popup_url in zip(referee_names, popup_urls, strict=False):
    ...
    def demonstrate_cache_performance(self):
        ...
        self.logger.data_info(
            f"First pass: {first_pass_time:.2f}s ({first_pass_ops} expensive ops)"
        )
        self.logger.data_info(
            f"Second pass: {second_pass_time:.2f}s ({second_pass_ops} expensive ops)"
        )
        ...
        self.logger.data_info(
            f"Cache hit rate: {100*self.cache_hits/(self.cache_hits+self.cache_misses):.1f}%"
        )
...
def demonstrate_advanced_caching():
    ...
    cache = PerformanceCache(strategy=CacheStrategy.MULTI_TIER, max_memory_size=100, default_ttl=60)
...
if __name__ == "__main__":
    ...
    example.logger.info("🚀 CACHE INTEGRATION DEMONSTRATION")
    example.logger.info("=" * 60)
    ...
    example.logger.success("Cache integration demonstration complete!")
    example.logger.info("Key benefits demonstrated:")
    example.logger.info("   - Multi-tier caching (memory + file)")
    example.logger.info("   - Extraction-specific cache helpers")
    example.logger.info("   - Function result caching")
    example.logger.info("   - Significant performance improvements")
    example.logger.info("   - Integrated logging and monitoring")
