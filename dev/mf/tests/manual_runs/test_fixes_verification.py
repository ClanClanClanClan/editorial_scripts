...
def test_orcid_client():
    ...
    try:
        ...
        test_search = {"name": "Dylan Possamai", "institution": "Columbia University"}
...
def test_extraction_components():
    ...
    try:
        ...
        print(
            f"✅ ORCID Client: {'YES' if hasattr(extractor, 'orcid_client') and extractor.orcid_client else 'NO'}"
        )
            ...
            "referees": [
                {
                    **enriched,  # Use enriched ORCID data
                    ...
                    if "University of Warwick" in str(enriched.get("affiliation_history", []))
                    else None,
                }
...
def analyze_fixes(manuscripts):
    ...
    print(
        f"📈 Data: {len(manuscripts)} manuscripts, {total_authors} authors, {total_referees} referees"
    )
    ...
    referees_with_hints = sum(
        1 for ms in manuscripts for r in ms.get("referees", []) if r.get("country_hints")
    )
