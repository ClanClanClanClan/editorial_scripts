...
class AcademicProfileEnricher:
    ...
    def _normalize_orcid(self, orcid_id: str) -> str:
        ...
        if len(orcid_clean) == 16 and "-" not in orcid_clean:
            ...
            orcid_clean = (
                f"{orcid_clean[:4]}-{orcid_clean[4:8]}-{orcid_clean[8:12]}-{orcid_clean[12:]}"
            )
    ...
    def enrich_person_profile(self, person_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if person_data.get("orcid") and self.check_orcid_quota():
            ...
            if orcid_data:
                ...
                print(
                    f"   ✅ ORCID data found: {len(orcid_data.get('publications', []))} publications"
                )

        # 2. Google Scholar enrichment (for metrics)
        if person_data.get("name"):
            scholar_data = self.get_scholar_profile(
                person_data["name"], person_data.get("institution", "")
            )
            if scholar_data:
        ...
        enriched_profile["enrichment_metadata"] = {
            ...
            "confidence_score": self._calculate_confidence_score(
                enriched_profile, enrichment_sources
            ),
    ...
    def get_orcid_profile(self, orcid_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if time_since_last < self.min_request_interval:
                sleep_time = (
                    self.min_request_interval - time_since_last + (0.1 * (hash(orcid_id) % 10))
                )  # Add jitter
            ...
            if response.status_code == 200:
                ...
                profile = {
                    ...
                    "last_modified": orcid_data.get("history", {})
    ...
    def get_scholar_profile(self, name: str, institution: str = "") -> dict[str, Any] | None:
    ...
    def _search_scholar_profile(self, name: str, institution: str) -> dict[str, Any] | None:
    ...
    def get_institution_data(self, institution_name: str) -> dict[str, Any] | None:
            ...
            if response.status_code == 200:
                ...
                if ror_data.get("items"):
                    ...
                    return {
                        ...
                        "city": best_match.get("addresses", [{}])[0].get("city")
                        if best_match.get("addresses")
                        else None,
    ...
    def _extract_orcid_name(self, orcid_data: dict) -> str | None:
    ...
    def _extract_orcid_biography(self, orcid_data: dict) -> str | None:
    ...
    def _extract_orcid_external_ids(self, orcid_data: dict) -> dict[str, str]:
        ...
        try:
            person = orcid_data.get("person", {})
            external_identifiers = person.get("external-identifiers", {}).get(
                "external-identifier", []
            )
    ...
    def _parse_orcid_date(self, date_data: dict | None) -> str | None:
    ...
    def _merge_orcid_data(self, profile: dict, orcid_data: dict) -> dict[str, Any]:
        ...
        if publications:
            ...
            updates["recent_publications"] = [
                p for p in publications if self._is_recent_publication(p)
            ]
            updates["publication_years"] = list(
                set(
                    [
                        self._extract_year_from_pub(p)
                        for p in publications
                        if self._extract_year_from_pub(p)
                    ]
                )
            )
    ...
    def _extract_year_from_pub(self, publication: dict) -> int | None:
...
def enrich_all_people_from_mf_data(mf_extraction_file: str) -> dict[str, Any]:
    ...
    try:
        with open(mf_extraction_file) as f:
            mf_data = json.load(f)
    except Exception as e:
