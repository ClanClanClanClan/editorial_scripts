...
print("🔥 ULTRA-COMPREHENSIVE TEST SUITE 🔥")
print("=" * 70)
...
class UltraComprehensiveTests:
    ...
    async def test_error_recovery(self):
        ...
        # Test 2.2: Recovery from invalid data
    ...
    async def test_schema_safety(self):
        ...
        # Test 6.2: Column types are correct
        column_checks = [
            ('referees_analytics', 'id', 'uuid'),
            ('referees_analytics', 'email', 'character varying'),
            ('referee_analytics_cache', 'metrics_json', 'json'),
    ...
    async def test_business_logic(self):
        ...
        # Test 7.1: Overall score calculation
        metrics = RefereeMetrics(
            referee_id=str(uuid.uuid4()),
            name="Score Test",
            email=f"score_{datetime.now().timestamp()}@test.com",
            institution="Score University",
            time_metrics=TimeMetrics(2, 14, 7, 21, 0.5, 3, 1.0),  # Perfect on-time
        ...
        # Test 7.2: Data completeness calculation
        ...
        if 0.3 <= completeness <= 0.7:  # Should be partially complete
            self.record_result(f"Data completeness ({completeness:.2f})", True)
        else:
    ...
    async def test_operational_readiness(self):
        ...
        # Test 8.1: Logging functionality
        import logging
        ...
        try:
            fake_id = uuid.uuid4()
            await self.repo.get_referee_metrics(fake_id)  # Should log error
            
            # Check if error was logged
            error_logs = [r for r in log_capture if r.levelname == 'ERROR']
            if error_logs:
                self.record_result("Error logging", True)
            else:
                self.record_result("Error logging", False, "No error logs captured")
        ...
        try:
            ...
            # Insert corrupted cache
            await self.conn.execute("""
                INSERT INTO referee_analytics_cache 
                (referee_id, metrics_json, calculated_at, valid_until, data_version)
                VALUES ($1, $2, $3, $4, $5)
            """, test_id, '{"invalid": json}', datetime.now(), datetime.now() + timedelta(hours=1), 1)
