                ...
                if category["count"] > 0:
                    ...
                    if take_action_links:
                        ...
                        for i, table in enumerate(all_tables):
                            try:
                                mailpopup_links = table.find_elements(
                        ...
                        if referee_table:
                            ...
                            rows_with_links = referee_table.find_elements(
                            ...
                            for row_idx, row in enumerate(rows_with_links, 1):
                                ...
                                for cell_idx, cell in enumerate(all_cells):
                                    ...
                                    print(
                                        f"   Cell {cell_idx}: '{cell_text[:100]}{'...' if len(cell_text) > 100 else ''}'"
                                    )
                                ...
                                if not found_status:
                                    print("   ❌ No status keywords found in row")

                                # Also check if there are review links in status area
                                review_links = row.find_elements(
                                    By.XPATH,
                                    ".//a[contains(@href, 'REVIEW') or contains(text(), 'review') or contains(text(), 'Review')]",
                                )
                                if review_links:
                                    ...
                                    print("   📝 No review links found in this row")
