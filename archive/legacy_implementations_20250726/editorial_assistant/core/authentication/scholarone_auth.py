...
class ScholarOneAuth(AuthenticationProvider):
    ...
    def _check_2fa_required(self, driver: WebDriver) -> bool:
        ...
        try:
            ...
            verification_selectors = [(By.ID, "TOKEN_VALUE"), (By.ID, "validationCode")]
    ...
    async def _handle_2fa_verification(self, driver: WebDriver) -> AuthenticationResult:
        ...
        try:
            ...
            return AuthenticationResult(
                status=AuthStatus.SUCCESS, message="2FA verification successful"
            )
    ...
    def _find_verification_input(self, driver: WebDriver):
        ...
        selectors = [(By.ID, "TOKEN_VALUE"), (By.ID, "validationCode")]

        for selector_type, selector in selectors:
            try:
                element = wait.until(
                    lambda d: d.find_element(selector_type, selector)
                    if self._element_exists_and_visible(d, selector_type, selector)
                    else None
                )
                if element:
