...
class JOTAJournal(JournalBase):
    def __init__(self, driver, debug=True):
        super().__init__(driver)
        self.debug = debug
        self.paper_downloader = get_paper_downloader()
        
        # For backwards compatibility, we still use gmail_service
        self.gmail_service = None
        self.user_id = "me"
        
    def get_url(self):
        return "https://www.editorialmanager.com/jota/"
        
    def set_gmail_service(self, gmail_service, user_id="me"):
        """
        Set the Gmail service for email-based operations
    ...
    def list_messages(self, query):
        ...
        messages = []
        request = self.gmail_service.users().messages().list(userId=self.user_id, q=query)
    ...
    def get_message(self, msg_id):
        ...
        msg = self.gmail_service.users().messages().get(userId=self.user_id, id=msg_id, format='full').execute()
