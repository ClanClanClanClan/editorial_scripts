            ...
            for category in categories:
                if category["count"] > 0:
                    ...
                    if take_action_links:
                        ...
                        # Get title
                        try:
                            title_elem = extractor.driver.find_element(
                                By.XPATH,
                        ...
                        try:
                            header_text = extractor.driver.find_element(
                                By.XPATH, "//td[@class='headerbg2']"
                            ).text
                            if "Submitted:" in header_text:
                                import re

                                submitted_match = re.search(
                                    r"Submitted:\s*(\d{1,2}-\w{3}-\d{4})", header_text
                                )
                                if submitted_match:
                                ...
                                updated_match = re.search(
                                    r"Last Updated:\s*(\d{1,2}-\w{3}-\d{4})", header_text
                                )
                                if updated_match:
                                ...
                                status_match = re.search(
                                    r"(Awaiting Reviewer Scores|Under Review|Completed)",
                                    header_text,
                                )
                                if status_match:
                        ...
                        try:
                            ...
                            author_line_elem = extractor.driver.find_element(
                                By.XPATH,
