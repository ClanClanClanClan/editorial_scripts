...
class JOTAExtractor(EditorialManagerExtractor):
    ...
    def extract_manuscripts(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logging.info(f"[{self.journal.code}] Extracted {len(merged_manuscripts)} manuscripts total")
    ...
    def _extract_from_web(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logging.info(f"[{self.journal.code}] Web extraction found {len(manuscripts)} manuscripts")
            return manuscripts
    ...
    def _extract_from_emails_only(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logging.info(f"[{self.journal.code}] Email-only extraction found {len(manuscripts)} manuscripts")
            return manuscripts
    ...
    def _collect_manuscripts_from_dashboard(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            manuscript_list_link = platform_config.get('navigation', {}).get('manuscript_list', 'a[href*="PendingTasks"]')
            
            manuscript_link = self._wait_for_element_by_css_selector(manuscript_list_link, timeout=10)
            if manuscript_link:
        ...
        try:
            ...
            for row in table.find_all("tr"):
                ...
                try:
                    ...
                    author = self._clean_author_name(tds[4].get_text(strip=True)) if len(tds) > 4 else ""
    ...
    def _get_referee_email(self, link_element, referee_name: str) -> str:
        ...
        try:
            ...
            logging.warning(f"[{self.journal.code}] Failed to get referee email for {referee_name}: {e}")
    ...
    def _merge_web_and_email_data(self, web_manuscripts: List[Dict[str, Any]], email_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for flagged_email in email_data.get("flagged_emails", []):
            ...
            if manuscript_id in merged:
                ...
                if flagged_email.get("type") == "acceptance" and flagged_email.get("referee_name"):
                    ...
                    self._update_referee_status(manuscript, flagged_email["referee_name"], "Accepted")
                
                elif flagged_email.get("type") == "invitation" and flagged_email.get("referee_name"):
                    # Add invited referee
                    self._update_referee_status(manuscript, flagged_email["referee_name"], "Invited")
        
        return list(merged.values())
    
    def _enhance_with_referee_emails(self, manuscripts: List[Dict[str, Any]], flagged_emails: List[Dict[str, Any]]) -> None:
    ...
    def _update_referee_status(self, manuscript: Dict[str, Any], referee_name: str, status: str) -> None:
