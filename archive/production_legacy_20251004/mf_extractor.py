...
class ComprehensiveMFExtractor(CachedExtractorMixin):
    ...
    def __init__(self):
        ...
        # Initialize ORCID client for enrichment
        if ORCID_AVAILABLE:
            try:
                self.orcid_client = ORCIDClient()
                print("   ✅ ORCID API client initialized")
            except Exception as e:
                print(f"   ⚠️ ORCID client initialization failed: {e}")
            ...
            if frames:
                ...
                for i in range(len(frames)):
                    try:
                        ...
                        if email_inputs:
                            value = self.safe_array_access(email_inputs, 0).get_attribute("value")
    ...
    def click_and_extract_email(self, name_link):
        ...
        try:
            ...
            self.driver.execute_script("self.safe_array_access(arguments, 0).click();", name_link)
            ...
            # Switch to popup
            popup_window = self.safe_array_access(windows, -1)
            ...
            try:
                ...
                for frame in frames[:3]:  # Only check first 3 frames
                    try:
                        ...
                        elements = self.safe_find_elements(By.NAME, "EMAIL_TEMPLATE_TO")
                        if elements:
                            value = self.safe_array_access(elements, 0).get_attribute("value")
                            if value and "@" in value:
                                email = value.strip()
                                break
                        self.driver.switch_to.default_content()
        ...
        try:
            ...
            for pattern in mf_patterns:
                ...
                if matches:
                    ...
                    manuscript_id = (
                        self.safe_array_access(matches, 0)
                        .upper()
                        .replace(" ", "-")
                        .replace(".", "-")
                        .replace("/", "-")
            ...
            for pattern in patterns:
                ...
                if matches:
                    manuscript_id = self.safe_array_access(matches, 0)
                            ...
                            if response.status_code == 200:
                                ...
                                if meta_match:
                                    ...
                                    for pattern in inst_patterns:
                                        ...
                                        if matches:
                                            found_institution = (
                                                self.safe_array_access(matches, 0).strip().title()
                                            )
                                            print(
                            ...
                            if response.status_code == 200:
                                ...
                                if abstract:
                                    ...
                                    for pattern in inst_patterns:
                                        ...
                                        if matches:
                                            found_institution = self.safe_array_access(
                                                matches, 0
                                            ).strip()
                                            print(
    ...
    def login(self):
        ...
        for attempt in range(MAX_LOGIN_ATTEMPTS):
            try:
                ...
                self.smart_wait(5)  # Give more time for initial page load
                ...
                try:
                    ...
                    # Record EXACT time when credentials are submitted (2FA email triggers NOW)
                    login_start_time = time.time()
                    print(
                        f"   ⏰ Credentials submitted at: {datetime.fromtimestamp(login_start_time).strftime('%H:%M:%S')}"
                    )
                ...
                try:
                    ...
                    # Use the timestamp from when credentials were submitted
                    print(
                        f"   📧 Fetching codes sent after: {datetime.fromtimestamp(login_start_time).strftime('%H:%M:%S')}"
                    )
                    ...
                    if code:
                        ...
                        # Wait for field to be interactable and enter code
                        try:
                            ...
                            self.driver.execute_script(
                                "document.getElementById('TOKEN_VALUE').value = '';"
                            )
                            ...
                            if entered_value != code:
                                print(
                                    f"   ⚠️ Code entry mismatch: entered '{entered_value}' vs expected '{code}'"
                                )
                                # Force it with JavaScript
                                self.driver.execute_script(
                                    f"document.getElementById('TOKEN_VALUE').value = '{code}';"
                                )
                                print("   🔄 Forced code entry with JavaScript")
                            else:
                                print(f"   ✅ Code entered correctly: {code}")
                        except Exception as e:
                        ...
                        # Find and click verify button - try multiple approaches
                        verify_btn = self.safe_find_element(By.ID, "VERIFY_BTN")
                        if verify_btn:
                            print("   🔍 Found VERIFY_BTN, attempting submission...")
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        print("   🔘 Clicked with direct JavaScript")
                                    except:
                                        print("   ❌ Could not click verify button")
                        else:
                            # Try alternative button selectors
                            print("   ⚠️ VERIFY_BTN not found, trying alternatives...")
                            # Try by class or text
                            verify_btn = self.safe_find_element(
                                By.XPATH,
                                "//button[contains(text(), 'Verify') or contains(text(), 'Submit') or contains(text(), 'Continue')]",
                            )
                            if verify_btn:
                                print("   ✅ Found verify button by text")
                                self.safe_click(verify_btn)
                            else:
                                # Try submitting the form directly
                                print("   🔄 No button found, trying form submission...")
                                try:
                                    token_field.send_keys(Keys.RETURN)
                                    print("   ↩️ Submitted with Enter key")
                                except:
                                    print("   ❌ Could not submit 2FA code")
                        print("   ⏳ Waiting for 2FA verification to complete...")
                        self.smart_wait(8)  # Increased wait time for verification

                        # Check if 2FA succeeded with better detection
                        print("   🔍 Checking 2FA result...")
                        current_url = self.driver.current_url
                        ...
                        # Check 2: Look for main page elements
                        if not success:
                            main_indicators = [
                                ...
                                (
                                    By.XPATH,
                                    "//div[@class='nav' or @class='menu' or contains(@class, 'dashboard')]",
                                ),
                            ]
                            for by_type, selector in main_indicators:
                                try:
                                    element = self.driver.find_element(by_type, selector)
                                    if element:
                                        print(f"   ✅ Found main page element: {selector}")
                                        success = True
                                        break
                                except:
                                    continue

                        # Check 3: TOKEN_VALUE field gone (original check)
                        if not success:
                            try:
                                still_on_2fa = self.safe_find_element(
                                    By.ID, "TOKEN_VALUE", timeout=2
                                )
                                print("   ❌ 2FA failed - still on verification page")
                            except:
                                print("   ✅ TOKEN_VALUE field gone - login successful")
                                success = True
                        ...
                        # DO NOT REFRESH - it loses the session!
        ...
        try:
            ...
            try:
                title_elem = info_table.find_element(
                    By.XPATH,
                    ".//self.safe_array_access(tr, 2)/td[@colspan='2']/p[@class='pagecontents']",
                )
            ...
            try:
                ...
                author_row = info_table.find_element(
                    By.XPATH,
                    ".//self.safe_array_access(tr, 3)/td[@colspan='2']/p[@class='pagecontents']",
                )
        ...
        try:
            ...
            for row_index, row in enumerate(referee_rows):
                ...
                try:
                    ...
                    if len(name_cells) > 1:
                        name_cell = self.safe_array_access(
                            name_cells, 1
                    ...
                    if not email:
                        try:
                            ...
                            if len(name_cells) > 1:
                                name_cell = self.safe_array_access(name_cells, 1)
                    ...
                    if (
                        ...
                        if parsed_affiliation.get("country_hints"):
                            ...
                            # Set the most likely country directly
                            if parsed_affiliation["country_hints"]:
                                referee["country"] = parsed_affiliation["country_hints"][0]
                    ...
                    try:
                        ...
                        try:
                            ...
                            if review_link:
                                print(
                                    "         📄 Found review report link - extracting with timeout protection"
                                )
                                # Extract report with timeout protection
                                report_data = self.extract_report_with_timeout(
                                    review_link, referee["name"], manuscript["id"]
                                )
                                if report_data:
                                    referee["report"] = report_data
                                    # Ensure recommendation is properly stored
                                    referee = self.ensure_recommendation_storage(
                                        referee, report_data
                                    )
                                    ...
                                    referee["report_link_href"] = (
                                        review_link.get_attribute("href") or ""
                                    )
                    ...
                    try:
                        ...
                        for date_row in date_rows:
                            try:
                                ...
                                if len(cells) >= 2:
                                    date_type = (
                                        self.safe_array_access(cells, 0)
                                        .text.strip()
                                        .lower()
                                        .replace(":", "")
                                    )
                    ...
                    try:
                        ...
                        if history_links:
                            report_url = self.safe_array_access(history_links, 0).get_attribute(
                                "href"
                            )
                            ...
                            if any(
                                status in referee_status
                                for status in ["received", "completed", "submitted", "returned"]
                            ):
                                print("         📄 Report submitted - downloading...")
                                try:
                                    pdf_path = self.download_referee_report_pdf(
                                        report_url, referee["name"], manuscript.get("id", "UNKNOWN")
                                    )
                                    if pdf_path:
                                        ...
                                        if report_text:
                                            referee["report"]["text_content"] = report_text[
                                                :1000
                                            ]  # First 1000 chars
                                            referee["report"][
                                                "recommendation"
                                            ] = self.extract_recommendation(report_text)
                                            referee["report"][
                                                "key_concerns"
                                            ] = self.extract_key_concerns(report_text)
                                            print("         ✅ Report downloaded and analyzed")
                                except Exception as e:
                                    print(f"         ⚠️ Report download failed: {e}")
                            else:
                                print(
                                    f"         ⏳ Report not submitted yet (status: {referee.get('status')})"
                                )
                        else:
                    ...
                    if referee.get("name"):
                        print(f"         🎓 ORCID PROFILE ENRICHMENT for {referee['name']}...")
                    ...
                    # Ensure country is set if we have country_hints
                    if referee.get("country_hints") and not referee.get("country"):
                        referee["country"] = referee["country_hints"][0]
                        print(f"         🌍 Setting country from hints: {referee['country']}")
                    ...
                    print(f"            🌍 Country: {referee.get('country', '❌')}")
    ...
    def extract_document_links(self, manuscript):
        ...
        try:
            ...
            if abstract_links:
                ...
                abstract_text = self.extract_abstract_from_popup(
                    self.safe_array_access(abstract_links, 0)
                )
                if abstract_text:
            ...
            if cover_links:
                ...
                cover_path = self.download_cover_letter(
                    self.safe_array_access(cover_links, 0), manuscript["id"]
                )
                if cover_path:
    ...
    def extract_missing_mor_fields(self, manuscript):
        ...
        try:
            ...
            for pattern in data_patterns:
                elements = self.driver.find_elements(By.XPATH, pattern)
                for elem in elements:
                    if "data availability" in self.safe_get_text(elem).lower():
                        manuscript["data_availability"] = self.safe_get_text(elem)
                        print(f"      ✅ Data Availability: {self.safe_get_text(elem)[:50]}...")
                        break
        ...
        try:
            ...
            # Method 1: Look for specific table cells
            try:
                rec_cells = self.driver.find_elements(
                    By.XPATH,
                    "//td[contains(text(), 'Recommended Reviewer') or contains(text(), 'Suggested Reviewer')]/following-sibling::td",
                )
                if rec_cells:
                    for cell in rec_cells:
                        text = self.safe_get_text(cell)
                        if text and text != "None" and len(text) > 5:
                            # Parse the text for names and emails
                            lines = text.split("\n")
                            for line in lines:
                                if line.strip():
                                    ...
                                    if email_match:
                                        ...
                                        manuscript["referee_recommendations"][
                                            "recommended_referees"
                                        ].append({"name": name, "email": email, "institution": ""})
                                    elif len(line.strip()) > 3:
                                        manuscript["referee_recommendations"][
                                            "recommended_referees"
                                        ].append(
                                            {"name": line.strip(), "email": "", "institution": ""}
                                        )
                            print("      ✅ Found recommended reviewers in table cells")
            except:
                pass

            # Method 2: Look for opposed/excluded reviewers in table cells
            try:
                opp_cells = self.driver.find_elements(
                    By.XPATH,
                    "//td[contains(text(), 'Opposed Reviewer') or contains(text(), 'Excluded Reviewer')]/following-sibling::td",
                )
                if opp_cells:
                    for cell in opp_cells:
                        ...
                        if text and text != "None" and len(text) > 5:
                            ...
                            print("      ✅ Found opposed reviewers in table cells")
            except:
                pass

            # Method 3: Look in the manuscript information section more broadly
            try:
                # Look for any cell with text containing "recommend" or "suggest" or "oppose"
                all_cells = self.safe_find_elements(By.XPATH, "//td")
                for cell in all_cells:
                    cell_text = self.safe_get_text(cell)
                    cell_lower = cell_text.lower()

                    # Check for recommended reviewers
                    if (
                        "recommend" in cell_lower or "suggest" in cell_lower
                    ) and "reviewer" in cell_lower:
                        # Get the next cell or text after this label
                        try:
                            next_cell = cell.find_element(
                                By.XPATH, "./following-sibling::self.safe_array_access(td, 1)"
                            )
                            reviewer_text = self.safe_get_text(next_cell)
                            if reviewer_text and reviewer_text != "None" and len(reviewer_text) > 5:
                                # Parse reviewers
                                lines = reviewer_text.split("\n")
                                for line in lines:
                                    if line.strip() and len(line.strip()) > 3:
                                        # Extract name and institution/email
                                        parts = line.split(",")
                                        if parts:
                                            name = self.safe_array_access(parts, 0).strip()
                                            institution = (
                                                self.safe_array_access(parts, 1).strip()
                                                if len(parts) > 1
                                                else ""
                                            )
                                            email = ""
                                            # Check for email in the line
                                            email_match = re.search(r"[\w\.-]+@[\w\.-]+\.\w+", line)
                                            if email_match:
                                                email = email_match.group()
                                                # Remove email from institution
                                                institution = institution.replace(email, "").strip()

                                            if (
                                                name
                                                and not manuscript["referee_recommendations"][
                                                    "recommended_referees"
                                                ]
                                            ):
                                                manuscript["referee_recommendations"][
                                ...
                                print("      ✅ Found recommended reviewers in adjacent cells")
                        except:
                            pass

                    # Check for opposed reviewers
                    if (
                        "oppose" in cell_lower or "exclude" in cell_lower
                    ) and "reviewer" in cell_lower:
                        # Get the next cell or text after this label
                        try:
                            next_cell = cell.find_element(
                            ...
                            if reviewer_text and reviewer_text != "None" and len(reviewer_text) > 5:
                                ...
                                print("      ✅ Found opposed reviewers in adjacent cells")
                        except:
                            pass
            except Exception as e:
                print(f"      ⚠️ Method 3 failed: {e}")

            # Method 4: Original regex pattern search as fallback
            page_text = self.driver.page_source
    ...
    def extract_comprehensive_mor_parity_fields(self, manuscript):
        ...
        # Navigate to Manuscript Information tab first for all fields
        try:
            print("      📋 Navigating to Manuscript Information tab for comprehensive data...")
            self.navigate_to_manuscript_information_tab()
        except Exception as e:
            print(f"      ⚠️ Could not navigate to info tab: {e}")
            ...
            for referee in manuscript.get("referees", []):
                ...
                if report_text:
                    ...
                    for pattern, score_type in score_patterns:
                        ...
                        if match:
                            review_data["scores"][score_type] = self.safe_int(match.group(1))
        ...
        try:
            ...
            for selector in ae_comment_selectors:
                try:
                    ...
                    if elements:
                        ae_comments = self.safe_array_access(elements, 0).text.strip()
                        manuscript["comprehensive_ae_comments"] = {
                            "text": ae_comments,
        ...
        try:
            ...
            for selector in topic_selectors:
                try:
                    elements = self.driver.find_elements(By.XPATH, selector)
                    if elements:
                        manuscript["topic_area"] = self.safe_array_access(elements, 0).text.strip()
        ...
        try:
            ...
            rec_editor_patterns = [
                r"Recommended self.safe_array_access(Editor, s)?:\s*([^\n]+)",
                r"Suggested self.safe_array_access(Editor, s)?:\s*([^\n]+)",
                r"Preferred self.safe_array_access(Editor, s)?:\s*([^\n]+)",
            ]
            ...
            opp_editor_patterns = [
                r"Opposed self.safe_array_access(Editor, s)?:\s*([^\n]+)",
                r"Excluded self.safe_array_access(Editor, s)?:\s*([^\n]+)",
            ]
        ...
        try:
            ...
            for selector in history_selectors:
                try:
                    elements = self.driver.find_elements(By.XPATH, selector)
                    if elements:
                        history_text = self.safe_array_access(elements, 0).text
            ...
            for selector in version_selectors:
                try:
                    ...
                    if elements:
                        ...
                        try:
                            self.safe_array_access(elements, 0).click()
                            self.smart_wait(2)
                        except:
                            pass
                        ...
                        version_rows = self.driver.find_elements(
                            By.XPATH, "//tr[contains(@class, 'version-row')]"
                        )
                            ...
                            try:
                                version_info = {
                                    "version": row.find_element(
                                        By.XPATH, ".//self.safe_array_access(td, 1)"
                                    ).text,
                                    "date": row.find_element(
                                        By.XPATH, ".//self.safe_array_access(td, 2)"
                                    ).text,
                                    "decision": row.find_element(
                                        By.XPATH, ".//self.safe_array_access(td, 3)"
                                    ).text
                                    if len(row.find_elements(By.XPATH, ".//td")) > 2
                                    else "",
                                    "documents": [],
                                }

                                # Look for document links in this version
                                doc_links = row.find_elements(
                                ...
                                manuscript["version_history_documents"].extend(
                                    version_info["documents"]
                                )
                            except:
                                continue

                        print(
                            f"      ✅ Found {len(manuscript['versions'])} versions with {len(manuscript['version_history_documents'])} documents"
                        )
                        break
                except:
                    continue
    ...
    def extract_manuscript_details_page(self, manuscript):
        ...
        try:
            ...
            if details_link:
                ...
                self.extract_all_documents(manuscript)
        ...
        try:
            ...
            for selector in table_selectors:
                try:
                    ...
                    table_text = self.safe_get_text(info_table)
                    if any(
    ...
    def extract_authors_from_details(self, manuscript):
        ...
        try:
            ...
            try:
                ...
                next_row = auth_inst_row.find_element(
                    By.XPATH, "./following-sibling::self.safe_array_access(tr, 1)"
                )
            ...
            for row in author_rows:
                ...
                for link in links:
                    ...
                    if (
                        ...
                        if (
                            len(parts) == 2
                            and self.safe_array_access(parts, 0).strip()
                            and self.safe_array_access(parts, 1).strip()
                            and len(self.safe_array_access(parts, 0).strip()) > 1
                            and len(self.safe_array_access(parts, 1).strip()) > 1
                        ):
                ...
                try:
                    ...
                    if "," in raw_name:
                        ...
                        if (
                            ...
                            author[
                                "name"
                            ] = f"{self.safe_array_access(parts, 1).strip()} {self.safe_array_access(parts, 0).strip()}"
                        else:
                            print(
                    ...
                    try:
                        ...
                        if (
                            ...
                            parent_td = link.find_element(
                                By.XPATH, "./ancestor::self.safe_array_access(td, 1)"
                            )
                    ...
                    # Extract ORCID if present - look for actual ORCID links first
                    author["orcid"] = ""

                    # Method 1: Look for actual ORCID link elements (like referee extraction)
                    try:
                        ...
                        for link in orcid_links:
                            ...
                            if href and "orcid.org" in href:
                                # Check if this ORCID is associated with this author
                                # Get parent cell to see context
                                parent_text = ""
                                try:
                                    parent = link.find_element(By.XPATH, "./ancestor::td")
                                    parent_text = self.safe_get_text(parent).lower()
                                except:
                                    parent_text = row_text.lower()

                                # Check if author name is nearby
                                author_name_parts = author["name"].lower().split()
                                if any(
                                    part in parent_text
                                    ...
                                    author["orcid"] = href
                                    print(f"         🆔 ORCID (from link): {author['orcid']}")
                                    break
                    except:
                        pass

                    # Method 2: Fallback to text-based extraction if no link found
                    if not author["orcid"]:
                        for line in lines:
                            if "orcid" in line.lower() or "https://orcid.org" in line:
                                # More flexible regex that catches various ORCID formats
                                orcid_patterns = [
                                ...
                                for pattern in orcid_patterns:
                                    orcid_match = re.search(pattern, line)
                                    if orcid_match:
                                        orcid_id = (
                                            orcid_match.group(1)
                                            if "/" not in orcid_match.group(0)
                                            else orcid_match.group(1)
                                        )
                                        # Less restrictive check - just verify it's in the same row as the author
                                        author["orcid"] = f"https://orcid.org/{orcid_id}"
                                        print(f"         🆔 ORCID (from text): {author['orcid']}")
                                        break
                                if author["orcid"]:
                                    break

                    # If no ORCID found on page, search via API
                    if not author["orcid"] and self.orcid_client and author.get("name"):
                        print(f"         🔍 Searching ORCID API for {author['name']}...")
                        api_orcid = self.search_orcid_api(author["name"], author.get("institution"))
                        if api_orcid:
                            author["orcid"] = api_orcid
                            author["orcid_discovered"] = True
                            print(f"         🆔 ORCID (from API): {api_orcid}")
            ...
            if enhanced_authors:
                ...
                for i, author in enumerate(enhanced_authors):
                    if author.get("name"):
                        ...
                        self.safe_array_access(enhanced_authors, i).update(enriched_author)

                        # Deep web search for missing institution
                        if not self.safe_array_access(enhanced_authors, i).get(
                            "institution"
                        ) and self.safe_array_access(enhanced_authors, i).get("email"):
                            deep_institution = self.deep_web_search_affiliation(
                                author["name"], self.safe_array_access(enhanced_authors, i)["email"]
                            )
                            if deep_institution:
                                self.safe_array_access(enhanced_authors, i)[
                                    "institution"
                                ] = deep_institution
                                print(
                        ...
                        if not self.safe_array_access(enhanced_authors, i).get(
                            "country"
                        ) and self.safe_array_access(enhanced_authors, i).get("institution"):
                            deep_country = self.deep_web_search_country(
                                author["name"],
                                self.safe_array_access(enhanced_authors, i)["institution"],
                                self.safe_array_access(enhanced_authors, i).get("email", ""),
                            )
                            if deep_country:
                                self.safe_array_access(enhanced_authors, i)[
                                    "country"
                        ...
                        if self.safe_array_access(enhanced_authors, i).get(
                            "institution"
                        ) and not self.safe_array_access(enhanced_authors, i).get("department"):
    ...
    def extract_metadata_from_details(self, manuscript):
        ...
        try:
            ...
            try:
                ...
                if data_elements:
                    data_availability = self.safe_array_access(data_elements, 0).text.strip()
                    ...
                    if data_elements:
                        manuscript["data_availability_statement"] = self.safe_array_access(
                            data_elements, 0
                        ).text.strip()
                        print(
                            f"      ✅ Data Availability (fallback): {self.safe_array_access(data_elements, 0).text.strip()[:50]}..."
                        )
            except Exception as e:
                print(f"      ⚠️ Could not extract data availability: {e}")
            ...
            try:
                ...
                for area in content_areas:
                    if "Files" in self.safe_get_text(area) and (
                        ".pdf" in self.safe_get_text(area)
                        or ".zip" in self.safe_get_text(area)
                        or ".tex" in self.safe_get_text(area)
                    ):
    ...
    def extract_revised_manuscripts(self, manuscript):
        ...
        try:
            ...
            revision_links = self.driver.find_elements(
                By.XPATH,
                ...
                try:
                    ...
                    if href and (".pdf" in href or ".docx" in href):
                        ...
                        version_match = re.search(
                            r"[Vv]ersion\s*(\d+)|[Rr]evision\s*(\d+)|R(\d+)", text
                        )
                        version = "unknown"
                        if version_match:
                            version = (
            ...
            track_changes_links = self.driver.find_elements(
                By.XPATH,
            ...
            for link in track_changes_links:
                try:
                    ...
                    if href:
                        ...
                        print(f"      ✅ Found track changes: {self.safe_get_text(link)}")
                except:
                    continue
    ...
    def extract_latex_source(self, manuscript):
        ...
        try:
            ...
            source_links = self.driver.find_elements(
                By.XPATH,
            ...
            supp_section = self.driver.find_elements(
                By.XPATH,
            ...
            for section in supp_section:
                ...
                for link in links:
                    ...
                    if "latex" in text or "source" in text or ".tex" in text:
                        ...
                        print(f"      ✅ Found LaTeX in supplementary: {self.safe_get_text(link)}")
                        return True
    ...
    def extract_all_documents(self, manuscript):
        ...
        try:
            ...
            doc_count = sum(
                [
                    1 if manuscript.get("cover_letter_url") else 0,
            ...
            print(f"      ✅ Extracted {doc_count} additional documents")
            manuscript["additional_document_count"] = doc_count
    ...
    def ensure_recommendation_storage(self, referee, report_data):
        ...
        if report_data and "recommendation" in report_data:
            ...
            if any(
                word in str(report_data.get("comments_to_editor", "")).lower()
    ...
    def extract_audit_trail(self, manuscript):
        ...
        try:
            ...
            try:
                with open(
                    f"debug_page_before_audit_{manuscript.get('id', 'unknown')}.html", "w"
                ) as f:
                    f.write(self.driver.page_source)
                print("   🐛 DEBUG: Saved page source for audit trail debugging")
            except:
                pass
            ...
            if audit_link:
                try:
                    ...
                    try:
                        with open(
                            f"debug_audit_trail_page_{manuscript.get('id', 'unknown')}.html", "w"
                        ) as f:
                            f.write(self.driver.page_source)
                        print("   🐛 DEBUG: Saved audit trail page source")
                    except:
                        pass
                ...
                try:
                    ...
                    if select_options:
                        ...
                        for option in select_options:
                            ...
                            if range_matches:
                                max_num = max(
                                    max_num, max(self.safe_int(num) for num in range_matches)
                                )
                    ...
                    if len(cells) >= 2:
                        ...
                        if has_email_icon:
                            ...
                            try:
                                ...
                                if popup_links:
                                    popup_href = self.safe_array_access(
                                        popup_links, 0
                                    ).get_attribute("href")
                    ...
                    try:
                        ...
                        if author_links:
                            ...
                            if len(self.driver.window_handles) > 1:
                                ...
                                try:
                                    # Email
                                    email_field = self.safe_find_element(
                ...
                try:
                    ...
                    for email in emails_found:
                        if email and "@" in email:
                            ...
                            if any(
                                pattern in email_lower
                                for pattern in ["wiley", "manuscript", "journal"] + editor_patterns
                            ):
                                continue
                    ...
                    if author_emails:
                        ...
                        for email in author_emails:
                            ...
                            if any(
                                part in email_local for part in author_name_parts if len(part) > 2
                            ):
                                best_email = email
                                break

                        if not best_email and author_emails:
                            best_email = self.safe_array_access(author_emails, 0)
                    ...
                    try:
                        to_field = self.safe_find_element(By.NAME, "EMAIL_TEMPLATE_TO")
                    ...
                    print("         ❌ No author email found in popup (only journal emails)")
                ...
                try:
                    if len(self.driver.window_handles) > 0:
                        self.driver.switch_to.window(
                            self.driver.self.safe_array_access(window_handles, 0)
                        )
                        print("   🆘 Emergency fallback to first window")
                except Exception:
    ...
    def download_cover_letter(self, cover_link, manuscript_id):
        ...
        try:
            ...
            if existing_files:
                print(
                    f"      ✅ Cover letter already exists: {self.safe_array_access(existing_files, 0).name}"
                )
                ...
                if not file_downloaded:
                    ...
                    try:
                        ...
                        if len(content_text) < 100 or "Files attached:" in content_text:
                            ...
                            paragraphs = self.safe_find_elements(
                                By.XPATH, "//p[string-length(text()) > 50]"
                            )
                            if paragraphs:
                                content_text = "\n\n".join(
                                    [
                                        self.safe_get_text(p)
                                        for p in paragraphs
                                        if self.safe_get_text(p)
                                    ]
                                )
    ...
    def _download_file_from_url(self, url: str, manuscript_id: str):
        ...
        try:
            ...
            # Download file
            response = requests.get(
                url, cookies=cookies, headers=headers, stream=True, timeout=30, allow_redirects=True
            )
            print(
            ...
            if response.status_code == 200:
                ...
                if file_size > 100:
                    with open(file_path, "rb") as f:
                        ...
                        if (
                            b"<html" in start_bytes.lower()
                            or b"<!doctype html" in start_bytes.lower()
                        ):
                            print("      ⚠️ Downloaded file appears to be HTML, not a document")
                ...
                print(f"      📝 Response content preview: {self.safe_get_text(response)[:200]}")
                return None
    ...
    def process_category(self, category):
        ...
        try:
            cookie_button = self.safe_find_element(By.ID, "onetrust-accept-btn-handler")
        ...
        # Don't navigate here - let the main loop handle it
        ...
        for i in range(manuscript_count):
            try:
                ...
                if manuscript_id in self.processed_manuscript_ids:
                    ...
                    # Still need to navigate to next for proper flow
                    if i < manuscript_count - 1:
                        self.navigate_next_document()
                    continue
                ...
                # Add to manuscripts list and mark as processed
        ...
        for i in range(manuscript_count - 1, -1, -1):
            try:
                ...
                if manuscript_id and manuscript_id not in ["UNKNOWN", "NAVIGATION_FAILED"]:
                    # Skip if this was a duplicate that wasn't processed
                    if manuscript_id not in self.processed_manuscript_ids:
                        print(f"   ⏭️ Skipping {manuscript_id} - was duplicate")
                        if i > 0:
                            self.navigate_previous_document()
                        continue
    ...
    def navigate_to_ae_center(self):
        ...
        print("\n🔙 Navigating back to AE Center...")
        try:
            # Multiple ways to get back to AE Center
            ae_selectors = [
                ...
                (By.XPATH, "//a[contains(@href,'AE_HOME')]"),
            ]

            ae_link = None
            for by_type, selector in ae_selectors:
                try:
                    ae_link = self.driver.find_element(by_type, selector)
                    if ae_link:
                        print(f"   ✅ Found AE Center link using {by_type}: {selector}")
                        break
                except:
                    continue

            if ae_link:
                self.driver.execute_script("arguments[0].scrollIntoView(true);", ae_link)
                self.smart_wait(1)
    ...
    def enrich_referee_profiles(self, manuscript):
        ...
        try:
            ...
            for referee in manuscript.get("referees", []):
                ...
                person_data = {
                    "name": referee["name"],
                    "orcid": referee.get("orcid", ""),  # May be empty
                    "institution": referee.get(
                        "institution_parsed", referee.get("affiliation", "")
                    ),
                    "email": referee.get("email", ""),
                }

                # Enrich profile - this will search for ORCID if not present
                if self.orcid_client:
                    enriched_profile = self.orcid_client.enrich_person_profile(person_data)

                    # Update referee with discovered ORCID if found
                    if enriched_profile.get("orcid") and not referee.get("orcid"):
                        referee["orcid"] = enriched_profile["orcid"]
                        referee["orcid_discovered"] = True
                        referee["orcid_confidence"] = enriched_profile.get("orcid_confidence", 0)
                        print(f"      🎯 Discovered ORCID: {referee['orcid']}")

                    # Update with publication data
                    if enriched_profile.get("publications"):
                        referee["publications"] = enriched_profile["publications"][:10]  # Top 10
                        referee["publication_count"] = enriched_profile.get(
                            "publication_count", len(enriched_profile["publications"])
                        )
                        publication_count += referee["publication_count"]
                        enriched_count += 1
                        print(f"      ✅ Found {referee['publication_count']} publications")

                    # Update with research interests
                    if enriched_profile.get("research_interests"):
                        referee["research_interests"] = enriched_profile["research_interests"]
                        print(
                            f"      🔬 Research interests: {', '.join(referee['research_interests'][:3])}"
                        )

                    # Update with metrics
                    if enriched_profile.get("metrics"):
                        referee["publication_metrics"] = enriched_profile["metrics"]
                        print(
                            f"      📊 Metrics: {enriched_profile['metrics'].get('total_publications', 0)} total publications"
                        )

                    # Update with affiliation history
                    if enriched_profile.get("affiliation_history"):
                        referee["affiliation_history"] = enriched_profile["affiliation_history"]

                    # Update with institution and department from current affiliation
                    if enriched_profile.get("institution") and not referee.get("institution"):
                        referee["institution"] = enriched_profile["institution"]
                        print(f"      🏛️ Institution from ORCID: {referee['institution']}")

                    if enriched_profile.get("department") and not referee.get("department"):
                        referee["department"] = enriched_profile["department"]
                        print(f"      🏢 Department from ORCID: {referee['department']}")

                    if enriched_profile.get("role"):
                        referee["current_role"] = enriched_profile["role"]
                        print(f"      👤 Current role: {referee['current_role']}")

                    # Update with country if found and not already set
                    if enriched_profile.get("country") and not referee.get("country"):
                        referee["country"] = enriched_profile["country"]
                        print(f"      🌍 Country from ORCID: {referee['country']}")

                    # Update with external IDs
                    if enriched_profile.get("other_ids"):
                        referee["external_ids"] = enriched_profile["other_ids"]

                    # Add enrichment metadata
                    referee["enrichment_date"] = enriched_profile.get("enrichment_date")
                    referee["enrichment_source"] = enriched_profile.get("enrichment_source")
            ...
            print(f"   ⚠️ Error during referee enrichment: {e}")
    ...
    def extract_all(self):
        ...
        while wait_count < max_wait:
            ...
            if "page=LOGIN" not in current_url and "login" not in current_url.lower():
                # Also check for positive indicators that we're logged in
                try:
                    # Check for elements that indicate successful login
                    self.safe_find_element(By.PARTIAL_LINK_TEXT, "Center")
                    print("   ✅ Found role center links - login confirmed")
                    break
                except:
                    pass

                # Even if URL looks good, wait a bit more
                if wait_count > 5:  # After 10 seconds, assume we're good
                    break

            print(f"   ⏳ Waiting for page transition... ({wait_count + 1}/{max_wait})")
        ...
        try:
            ...
            all_links = self.safe_find_elements(By.TAG_NAME, "a")
            text_links = [
                self.safe_get_text(link) for link in all_links[:15] if self.safe_get_text(link)
            ]
        ...
        # MF SPECIFIC CATEGORIES - Exact list from AE Center
        print("   📋 Checking for MF manuscript categories...")
        clickable_categories = []

        # Exact MF categories as they appear on AE Center
        mf_categories = [
        ...
        # Try to find each specific category
        for category_name in mf_categories:
            try:
                # Try exact text match
                element = None
                try:
                    element = self.driver.find_element(By.LINK_TEXT, category_name)
                except:
                    # Try XPath if link text doesn't work
                    try:
                        element = self.driver.find_element(
                            By.XPATH, f"//a[text()='{category_name}']"
                        )
                    except:
                        # Try normalized space
                        try:
                            element = self.driver.find_element(
                                By.XPATH, f"//a[normalize-space(text())='{category_name}']"
                            )
                ...
                if element and element.is_displayed():
                    ...
                    # Try to find manuscript count
                    count = 0
                    try:
                        # Look for count in parent row
                        parent_row = element.find_element(
                            By.XPATH, "./ancestor::self.safe_array_access(tr, 1)"
                        )
                        # Look for bold numbers
                    ...
                    clickable_categories.append(
                        {"name": category_name, "element": element, "href": href, "count": count}
                    )

                    if count > 0:
                        print(f"   ✅ Found: {category_name} ({count} manuscripts)")
                    else:
                        print(f"   ✅ Found: {category_name} (0 manuscripts)")
                else:
                    print(f"   ⚠️ Not found: {category_name}")

            except Exception:
                print(f"   ⚠️ Could not find category '{category_name}'")
                continue

        if not clickable_categories:
            print("   ⚠️ No MF categories found, trying dynamic detection...")
            # Fallback to dynamic detection
            all_links = self.safe_find_elements(By.TAG_NAME, "a")
            for link in all_links:
                try:
                    link_text = self.safe_get_text(link)
                    if any(cat in link_text for cat in mf_categories):
                        href = link.get_attribute("href")
                        if href and link.is_displayed():
                            clickable_categories.append(
                                {"name": link_text, "element": link, "href": href, "count": 0}
                            )
                            print(f"   ✅ Found (dynamic): {link_text}")
                except:
                    continue

        # Filter to only categories with manuscripts (count > 0)
        categories_with_manuscripts = [
            cat for cat in clickable_categories if cat.get("count", 0) > 0
        ]

        if categories_with_manuscripts:
            categories = categories_with_manuscripts
            print(f"\n   📊 Processing {len(categories)} categories with manuscripts:")
            for cat in categories:
                print(f"      → {cat['name']}: {cat['count']} manuscripts")
        else:
            print("\n   ℹ️ No categories have manuscripts currently")
            print("   📋 Categories checked:")
            for cat in clickable_categories:
                print(f"      → {cat['name']}: {cat.get('count', 0)} manuscripts")
            categories = []
        ...
        # Store category names for processing
        category_names = []
        for cat in categories:
            if isinstance(cat, dict):
                category_names.append(cat.get("name", "Unknown"))
            else:
                category_names.append(str(cat))

        for i, category_name in enumerate(category_names, 1):
            print(f"\n📦 CATEGORY {i}/{len(category_names)}: {category_name}")
            print("=" * 50)

            # If not first category, we need to re-find the link
            if i > 1:
                print(f"   🔍 Re-finding category link for: {category_name}")

                # Re-find the category link on AE Center page
                category_link = None
                try:
                    ...
                    try:
                        ...
                        print(f"   ❌ Could not find category link for {category_name}")
                        continue

                if category_link:
                    # Create fresh category dict
                    category = {
                        "name": category_name,
                ...
                # First category, use original
                category = self.safe_array_access(categories, 0)

            # Process this category
            self.process_category(category)

            # After processing, return to AE Center for next category (except last)
            if i < len(category_names):
                print("\n🔄 Returning to AE Center for next category...")

                # Navigate back to AE Center
                success = self.navigate_to_ae_center()

                if not success:
                    print("   ⚠️ Could not return to AE Center, trying alternative approach...")
                    ...
                    try:
                        ...
                        print("   ❌ Browser back also failed")

                # Verify we're back at AE Center
                self.smart_wait(2)  # Let page settle
                current_url = self.driver.current_url
                if "ASSOCIATE_EDITOR" in current_url or "AE_HOME" in current_url:
                    print("   ✅ Confirmed: Back at AE Center")
                else:
                    print(f"   ⚠️ May not be at AE Center. URL: {current_url}")

        print(f"\n✅ COMPLETED ALL {len(category_names)} CATEGORIES")

        # Show deduplication summary
        if self.processed_manuscript_ids:
            print("\n📊 EXTRACTION SUMMARY:")
            print(f"   Categories processed: {len(category_names)}")
            print(f"   Unique manuscripts: {len(self.processed_manuscript_ids)}")
            print(f"   Total encounters: {sum(cat.get('count', 0) for cat in categories)}")

            # Check for duplicates across categories
            all_ids_encountered = []
            for cat in categories:
                if "manuscript_ids" in cat:
                    all_ids_encountered.extend(cat["manuscript_ids"])

            if all_ids_encountered:
                duplicates = len(all_ids_encountered) - len(set(all_ids_encountered))
                if duplicates > 0:
        ...
        return {
            "manuscripts": {m["id"]: m for m in self.manuscripts},
    ...
    def _print_precise_results_summary(self):
        ...
        if manuscript_dir.exists():
            ...
            print("   ❌ No manuscripts directory")
            ...
            for cover in cover_files:
                if cover.is_file():
                    ...
                    file_type = (
                        ...
                        if cover.suffix == ".docx"
                        else "TEXT"
                    )
                    print(
                        f"      {'✅' if cover.suffix in ['.pdf', '.docx'] else '📝'} {cover.name} ({file_type}, {size_kb:.1f} KB)"
                    )
        else:
            print("   ❌ No cover letters directory")

        # Summary counts
        total_referees = sum(len(ms.get("referees", [])) for ms in self.manuscripts)
        total_pdfs = (
            len(list(Path("downloads/manuscripts").glob("*.pdf")))
            if Path("downloads/manuscripts").exists()
            else 0
        )
        total_covers = (
            len(list(Path("downloads/cover_letters").glob("*")))
            if Path("downloads/cover_letters").exists()
            else 0
        )
    ...
    def deep_web_enrichment(self, person_name, current_data=None):
        """ORCID-powered comprehensive enrichment for author/referee data."""
        if not person_name:
            return current_data or {}

        print(f"      🌐 ORCID ENRICHMENT for: {person_name}")
        ...
        # 2. USE ORCID API FOR COMPREHENSIVE ENRICHMENT
        if hasattr(self, "orcid_client") and self.orcid_client:
            try:
                # Prepare data for ORCID search
                person_data = {
                    "name": corrected_name or person_name,
                    "institution": enriched_data.get(
                        "institution_parsed", enriched_data.get("affiliation", "")
                    ),
                    "email": enriched_data.get("email", ""),
                    "orcid": enriched_data.get("orcid", ""),
                }

                # Get enriched profile from ORCID
                orcid_enriched = self.orcid_client.enrich_person_profile(person_data)

                # Update with ORCID data
                if orcid_enriched.get("orcid"):
                    enriched_data["orcid"] = orcid_enriched["orcid"]
                    if orcid_enriched.get("orcid_confidence"):
                        enriched_data["orcid_discovered"] = True
                        enriched_data["orcid_confidence"] = orcid_enriched["orcid_confidence"]
                        print(
                            f"      ✅ ORCID discovered: {orcid_enriched['orcid']} (confidence: {orcid_enriched['orcid_confidence']:.0%})"
                        )

                if orcid_enriched.get("publications"):
                    enriched_data["publications"] = orcid_enriched["publications"][:20]  # Top 20
                    enriched_data["publication_count"] = len(orcid_enriched["publications"])
                    print(f"      📚 Found {len(orcid_enriched['publications'])} publications")

                    # Show sample publications
                    for pub in orcid_enriched["publications"][:3]:
                        print(
                            f"         - {pub.get('year', 'N/A')}: {pub.get('title', 'Unknown')[:60]}..."
                        )

                if orcid_enriched.get("affiliation_history"):
                    enriched_data["affiliation_history"] = orcid_enriched["affiliation_history"]
                    current_affiliations = [
                        a for a in orcid_enriched["affiliation_history"] if a.get("current")
                    ]
                    if current_affiliations:
                        current = self.safe_array_access(current_affiliations, 0)
                        if not enriched_data.get("institution"):
                            enriched_data["institution"] = current["organization"]
                            print(f"      🏛️ Current institution: {current['organization']}")
                        if current.get("department") and not enriched_data.get("department"):
                            enriched_data["department"] = current["department"]
                            print(f"      🏢 Department: {current['department']}")
                        if current.get("role"):
                            enriched_data["current_role"] = current["role"]
                            print(f"      👤 Role: {current['role']}")

                if orcid_enriched.get("research_interests"):
                    enriched_data["research_interests"] = orcid_enriched["research_interests"]
                    print(
                        f"      🔬 Research interests: {', '.join(orcid_enriched['research_interests'][:5])}"
                    )

                if orcid_enriched.get("other_ids"):
                    enriched_data["external_ids"] = orcid_enriched["other_ids"]
                    if "scopus-author-id" in orcid_enriched["other_ids"]:
                        print(
                            f"      🆔 Scopus ID: {orcid_enriched['other_ids']['scopus-author-id']}"
                        )

                # Add institution/department/country from ORCID if not already set
                if orcid_enriched.get("institution") and not enriched_data.get("institution"):
                    enriched_data["institution"] = orcid_enriched["institution"]
                    print(f"      🏛️ Institution from ORCID: {orcid_enriched['institution']}")

                if orcid_enriched.get("department") and not enriched_data.get("department"):
                    enriched_data["department"] = orcid_enriched["department"]
                    print(f"      🏢 Department from ORCID: {orcid_enriched['department']}")

                if orcid_enriched.get("country") and not enriched_data.get("country"):
                    enriched_data["country"] = orcid_enriched["country"]
                    print(f"      🌍 Country from ORCID: {orcid_enriched['country']}")

                if orcid_enriched.get("metrics"):
                    enriched_data["publication_metrics"] = orcid_enriched["metrics"]
                    metrics = orcid_enriched["metrics"]
                    if metrics.get("total_publications"):
                        print(
                            f"      📊 Metrics: {metrics['total_publications']} publications over {metrics.get('years_active', 0)} years"
                        )

            except Exception as e:
                print(f"      ⚠️ ORCID enrichment error: {e}")

        # 3. FALLBACK: Try MathSciNet search (simulated)
        else:
            mathscinet_data = self.search_mathscinet(corrected_name or person_name)
            if mathscinet_data:
                enriched_data["mathscinet_id"] = mathscinet_data.get("mr_author_id")
                print(f"      📚 MathSciNet ID: {mathscinet_data.get('mr_author_id')}")

            # Try Google Scholar (simulated)
    ...
    def search_mathscinet(self, name):
        ...
        if name in mathscinet_db:
            return self.safe_array_access(mathscinet_db, name)
    ...
    def get_official_institution_name(self, institution):
        ...
        if institution_lower in corrections:
            return self.safe_array_access(corrections, institution_lower)
    ...
    def extract_timeline_analytics(self, manuscript):
        ...
        if event_dates:
            event_dates.sort()
            span = (
                self.safe_array_access(event_dates, -1) - self.safe_array_access(event_dates, 0)
            ).days
        ...
        invitation_events = [
            e for e in timeline if "invite" in str(e.get("description", "")).lower()
        ]
        response_events = [
            e
            for e in timeline
            if "accept" in str(e.get("description", "")).lower()
            or "agree" in str(e.get("description", "")).lower()
        ]
            ...
            for inv in invitation_events:
                ...
                if inv_date:
                    ...
                    later_responses = [
                        ...
                        if r.get("parsed_date") and r["parsed_date"] > inv_date
                    ]
                    if later_responses:
        ...
        reminder_events = [
            e for e in timeline if "reminder" in str(e.get("description", "")).lower()
        ]
        if reminder_events:
            ...
            for reminder in reminder_events:
                ...
                if reminder_date:
                    ...
                    responses_after = [
                        e
                        for e in timeline
                        if e.get("parsed_date")
                        and reminder_date < e["parsed_date"] <= cutoff_date
                        and (
                            "response" in str(e.get("description", "")).lower()
                            or "submit" in str(e.get("description", "")).lower()
                        )
                    ]
                    if responses_after:
        ...
        print(
            f"      ✅ Timeline analytics extracted: {analytics['total_events']} events, {analytics['communication_span_days']} days span"
        )
        return analytics
    ...
    def extract_version_history(self, manuscript):
        ...
        try:
            ...
            if not is_revision:
                print("         ⏭️ Skipping version history for original submission")
                return
            ...
            # Look for version history section
            version_sections = self.driver.find_elements(
                By.XPATH,
                "//td[contains(text(), 'Version History')]/following-sibling::td | //p[contains(text(), 'Version History')]/following::td[@class='dataentry']",
            )

            if not version_sections:
                print("         ⚠️ No version history section found for revision manuscript")
                return

            print("         📜 Extracting version history for revision...")
            ...
            for section in version_sections:
                try:
                    ...
                    version_links = section.find_elements(
                        By.XPATH, ".//a[contains(@href, 'MANUSCRIPT_DETAILS')]"
                    )
    ...
    def deep_web_search_affiliation(self, author_name, author_email):
        ...
        try:
            ...
            if cache_key in self._affiliation_search_cache:
                cached_affiliation = self.self.safe_array_access(
                    _affiliation_search_cache, cache_key
                )
            ...
            if author_email and "@" in author_email:
                ...
                if domain in domain_to_institution:
                    found_affiliation = self.safe_array_access(domain_to_institution, domain)
                ...
                if ".edu" in domain or ".ac." in domain:
                    ...
                    if parts:
                        potential_name = (
                            self.safe_array_access(parts, 0).upper()
                            if len(self.safe_array_access(parts, 0)) <= 4
                            else self.safe_array_access(parts, 0).title()
                        )
    ...
    def extract_referee_report_content(self, referee, manuscript_id):
        ...
        try:
            ...
            print("         📄 Checking for referee report PDF...")
            ...
            try:
                ...
                pdf_links = self.driver.find_elements(
                    By.XPATH, "//a[contains(@href, 'DOWNLOAD=TRUE')]"
                )
                ...
                if pdf_links:
                    ...
                    if pdf_path:
                        ...
                        if pdf_text:
                            referee["report"]["pdf_text"] = pdf_text[
                                :5000
    ...
    def extract_historical_referee_reports(self, manuscript):
        ...
        print("      📜 Extracting historical referee reports for revision...")

        manuscript["historical_reports"] = []

        try:
            # Navigate to version history section
            version_links = self.driver.find_elements(
                By.XPATH, "//a[contains(@href, 'MANUSCRIPT_DETAILS') and contains(text(), 'MAFI-')]"
            )
            ...
            for link in version_links:
                try:
                    version_id = self.safe_get_text(link)
                    ...
                    # Switch to new tab
                    self.driver.switch_to.window(
                        self.driver.self.safe_array_access(window_handles, -1)
                    )
                    ...
                    if historical_referees:
                        manuscript["historical_reports"].append(
                            {"version": version_id, "referees": historical_referees}
                        )
                        print(
                            f"         ✅ Extracted {len(historical_referees)} historical referees from {version_id}"
                        )
        ...
        try:
            ...
            for row in referee_rows:
                try:
                    ...
                    referee["name"] = self.safe_get_text(name_link)

                    # Get status
                    cells = row.find_elements(By.TAG_NAME, "td")
                    if len(cells) >= 3:
                        referee["status"] = self.safe_array_access(cells, 2).text.strip()
    ...
    def search_orcid_api(self, author_name, institution=None):
        ...
        try:
            if not self.orcid_client:
                return None

            # Use the ORCID client to search
            matches = self.orcid_client.search_by_name_and_affiliation(
                name=author_name, affiliation=institution
            )

            if matches:
                # Return the best match (highest score)
                best_match = self.safe_array_access(matches, 0)
                orcid_id = best_match.get("orcid")
                confidence = best_match.get("score", 0)
                if orcid_id:
                    print(
                        f"         🎯 ORCID API match found: {orcid_id} (confidence: {confidence:.0%})"
                    )
                    return f"https://orcid.org/{orcid_id}"
                else:
                    print("         ℹ️ ORCID API returned match but no ORCID ID")
            else:
                print(f"         ℹ️ No ORCID matches found for {author_name}")
    ...
    def search_crossref_api(self, author_name, manuscript_title=None):
        ...
        try:
            ...
            params = {"query.author": author_name, "rows": 5}
    ...
    def deep_web_search_email(self, person_name, affiliation):
        ...
        if cache_key in self._email_search_cache:
            return self.self.safe_array_access(_email_search_cache, cache_key)

        # TODO: Implement real academic email search using APIs
        # For now, return None to avoid fake data
        print("         ⚠️ Deep web search not implemented - would need real APIs")
        return None
