...
class NACOExtractor(MSPExtractor):
        ...
        try:
            ...
            for attempt in range(5):
                if mine_link.is_displayed() and mine_link.is_enabled():
                    try:
                        mine_link.click()
                        logging.info(f"[{self.journal.code}] Clicked 'Mine' link on attempt {attempt + 1}")
                        return
    ...
    def _parse_manuscript_from_article(self, article) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if manuscript_data["Manuscript #"] or manuscript_data["Title"]:
                logging.debug(f"[{self.journal.code}] Parsed manuscript: {manuscript_data['Manuscript #']} - {manuscript_data['Title']}")
