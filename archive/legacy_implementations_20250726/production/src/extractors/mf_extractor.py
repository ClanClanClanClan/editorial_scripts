    ...
    def get_email_from_popup_safe(self, popup_url):
        ...
        try:
            ...
            self.driver.execute_script(
                f"window.open('{popup_url}', 'popup', 'width=600,height=400')"
            )
...
class ComprehensiveMFExtractor:
    ...
    def get_current_manuscript_id(self):
        """Extract the current manuscript ID from the page."""
        try:
            page_text = self.driver.find_element(By.TAG_NAME, "body").text
            mafi_match = re.search(r"MAFI-\d{4}-\d{4}", page_text)
            if mafi_match:
                return mafi_match.group(0)
            else:
                print("   ⚠️ No MAFI manuscript ID found on current page")
                return "UNKNOWN"
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                code = fetch_latest_verification_code(
                    "MF", max_wait=45, poll_interval=3, start_timestamp=login_start_time
                )
    ...
    def get_manuscript_categories(self):
        ...
        for category_name in category_names:
            try:
                ...
                categories.append({"name": category_name, "count": count, "link": category_link})
        ...
        try:
            ...
            try:
                            ...
                            manuscript["authors"].append(
                                {
                                    ...
                                    "email": self.get_email_from_popup(author_link["href"])
                                    if author_link and "href" in author_link.attrs
                                    else "",  # Extract author email
                                }
                            )
                else:
                    # Single author case
                ...
                for link in author_links:
                    ...
                    if name and not any(
                        ed_name in name for ed_name in editor_names + referee_names
                    ):
                        is_contact = False
                        try:
                ...
                if ":" in text:
                    ...
                    try:
                        ...
                        manuscript["editors"][role] = {
                            "name": name.strip(),
                            "email": self.get_email_from_popup(editor_href)
                            if editor_href and "mailpopup" in editor_href
                            else "",
                        }
                    except:
                        manuscript["editors"][role] = {
                ...
                if response.status_code == 200:
                    ...
                    for country, patterns in country_patterns.items():
                        if any(pattern in abstract for pattern in patterns):
                            print(f"         🌍 Web search found: {institution_name} → {country}")
    ...
    def extract_referees_comprehensive(self, manuscript):
        ...
        try:
            ...
            referee_table_rows = self.driver.find_elements(
                By.XPATH,
            ...
            for row_index, row in enumerate(referee_table_rows):
                ...
                try:
                    ...
                    try:
                        ...
                        print(
                            f"         ⚠️ Could not get email for {referee['name']}: {str(e)[:100]}"
                        )
                            ...
                            for selector in affil_selectors:
                                try:
                                    ...
                                    for elem in affil_elements:
                                        ...
                                        if (
                                            affil_text
                                            and affil_text != referee["name"]
                                            and len(affil_text) > 3
                                        ):
                        ...
                        try:
                            ...
                            if "@" in email:
                                ...
                                email_domain_mappings = {
                                    ...
                                    "univ-lemans.fr": "Le Mans Université",
                                    "sorbonne-universite.fr": "Sorbonne Université",
                                    "upmc.fr": "Sorbonne Université",
                                ...
                                if domain in email_domain_mappings:
                                    ...
                                    print(f"         ❌ Domain {domain} not in mappings")
                        except Exception as e:
                    ...
                    try:
                        ...
                        try:
                            ...
                            if review_link:
                                print("         📄 Found review report link, extracting...")
                                ...
                                if report_data:
                                    ...
                                    if (
                                        report_data.get("url")
                                        and "history_popup" in report_data["url"]
                                    ):
                                        print("         🪟 Extracting popup content...")
                                        popup_content = self.extract_review_popup_content(
                                            report_data["url"], referee["name"]
                                        )
                                        if popup_content:
                                            referee["popup_review_content"] = popup_content
                                            print("         ✅ Extracted popup review content")

                                            # Parse structured recommendation
                                            structured_rec = self.parse_recommendation_from_popup(
                                                popup_content
                                            )
                                            if structured_rec:
                                                referee[
                                                ...
                                                print(
                                                    f"         ⭐ Recommendation: {structured_rec}"
                                                )
                        except:
                    ...
                    try:
                        ...
                        if history_links:
                            referee["report"] = {
                                "available": True,
                                "url": history_links[0].get_attribute("href"),
                            }
                    except:
                        pass
                    ...
                    print(
                        f"         ✅ {referee['name']} ({referee['status']}) - {referee['affiliation']}"
                    )
    ...
    def extract_authors_from_details(self, manuscript):
        ...
        try:
            print("      🔍 Looking for complete author table with all details...")
            ...
            for email in target_emails:
                try:
                    ...
                    if email_rows:
                        ...
                        print(f"      📧 Processing author with email: {email}")
                        ...
                        for line in lines:
                            if any(
                                keyword in line.lower()
                                for keyword in ["university", "institute", "department", "school"]
                            ):
                        ...
                        if "china" in row_text.lower():
                            ...
                            term in row_text.lower() for term in ["uk", "united kingdom", "britain"]
                        ):
                            author["country"] = "United Kingdom"
                        elif any(
                            term in row_text.lower() for term in ["usa", "united states", "america"]
                        ):
                        ...
                        print(f"      ✅ Author {authors_found}: {author['name']}")
                        print(f"         📧 Email: {author['email']}")
                        print(f"         🏛️ Institution: {author['institution']}")
                        print(f"         🌍 Country: {author['country']}")
                        print(f"         📝 Corresponding: {author['is_corresponding']}")
                        if author.get("orcid"):
                            print(f"         🆔 ORCID: {author['orcid']}")

                    else:
                        print(f"      ⚠️ No rows found for email: {email}")

                except Exception as e:
                    print(f"      ❌ Error processing email {email}: {str(e)[:100]}")
                    continue

            if enhanced_authors:
                manuscript["authors"] = enhanced_authors
                print(
                    f"      ✅ Successfully extracted {len(enhanced_authors)} authors with complete details"
                )
                return enhanced_authors

            # Fallback: Try original method for backward compatibility
            print("      ⚠️ Complete author table not found, trying fallback method...")

            # Look for Authors & Institutions section
            authors_section = self.driver.find_elements(
                By.XPATH, "//td[contains(text(), 'Authors & Institutions:')]/following-sibling::td"
            )

            if authors_section:
                print("      ✅ Found Authors & Institutions section (fallback)")
                ...
                author_rows = authors_table.find_elements(
                    By.XPATH, ".//tr[td//a[contains(@href, 'mailpopup')]]"
                )
                print(f"      📊 Found {len(author_rows)} author rows with basic data")
                ...
                for i, row in enumerate(author_rows):
                    try:
                        ...
                        if author.get("name"):
                            ...
                            print(
                                f"         🏛️ Institution: {author.get('institution', 'Not found')}"
                            )
                            print(
                                f"         📝 Corresponding: {author.get('is_corresponding', False)}"
                            )
                            if author.get("orcid"):
                                print(f"         🆔 ORCID: {author['orcid']}")

                    except Exception as e:
                        print(f"      ❌ Error processing author row {i+1}: {e}")
                        continue

                if enhanced_authors:
                    manuscript["authors"] = enhanced_authors
                    print(
                        f"      ✅ Updated manuscript with {len(enhanced_authors)} authors (fallback method)"
                    )
                    return enhanced_authors
                else:
                    print("      ❌ No authors extracted from details page")

            print("      ❌ No author data found in any format")
            return []

        except Exception as e:
            print(f"      ❌ Error extracting authors from details: {e}")
            import traceback
    ...
    def get_total_audit_events(self):
        ...
        try:
            ...
            pagination_text = self.driver.find_elements(
                By.XPATH, "//td[contains(text(), 'of') and contains(@class, 'pagecontents')]"
            )
            ...
            select_options = self.driver.find_elements(
                By.XPATH, "//select[@name='page_select']//option"
            )
    ...
    def extract_events_from_current_page(self):
        ...
        try:
            ...
            comm_rows = self.driver.find_elements(
                By.XPATH, "//tr[.//img[@src='/images/en_US/icons/letter.gif']]"
            )
            ...
            for i, row in enumerate(comm_rows):
                try:
                    ...
                    if len(cells) >= 2:
                        ...
                        for line in event_text.split("\n"):
                            ...
                            if line.startswith("To:"):
                                ...
                                communication["delivery_status"] = line.replace(
                                    "Results:", ""
                                ).strip()
                            elif line.startswith("Template Name:"):
                                communication["template"] = line.replace(
                                    "Template Name:", ""
                                ).strip()
                    ...
                    print(f"         ❌ Error processing communication row: {e}")
                    continue
    ...
    def execute_3_pass_extraction(self, category):
        ...
        # First click the category to open it
        category["link"].click()
        time.sleep(3)
        ...
        manuscript_count = len(take_action_links)
        manuscript_ids = []
        ...
        for i in range(manuscript_count):
            try:
                # Click on manuscript i
                take_action_links[i].click()
                time.sleep(5)
                ...
                if i < manuscript_count - 1:
                    ...
                    if not success:
                        # Go back to category and get fresh links
                        self.driver.back()
                        time.sleep(3)
                        take_action_links = self.driver.find_elements(
                            By.XPATH, "//a[contains(@href,'ASSOCIATE_EDITOR_MANUSCRIPT_DETAILS')]"
        ...
        for i in range(manuscript_count - 1, -1, -1):
            try:
                ...
                if manuscript_id and manuscript_id != "UNKNOWN":
                    ...
                    if manuscript:
                        # Extract manuscript information from details tab
                        self.extract_manuscript_details_page(manuscript)
