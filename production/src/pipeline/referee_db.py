...
class RefereeDB:
        ...
        with self._lock:
            ...
            conn.execute(
                """CREATE TABLE IF NOT EXISTS model_predictions (
                    ...
                    journal TEXT,
                    manuscript_id TEXT,
                    predicted_p_accept REAL,
                    predicted_p_complete REAL,
                    actual_accepted INTEGER,
                    actual_completed INTEGER,
                    actual_review_days INTEGER,
                    predicted_at TEXT,
                    resolved_at TEXT
                )"""
            )
    ...
    def get_assignments_by_manuscript(self, journal: str, manuscript_id: str) -> list[dict]:
        with self._lock:
            with self._connection() as conn:
                rows = conn.execute(
                    """SELECT * FROM referee_assignments
                       WHERE journal=? AND manuscript_id=?""",
    ...
    def get_recently_invited(self, days: int = 60) -> list[dict]:
        ...
        with self._lock:
            with self._connection() as conn:
                rows = conn.execute(
                    """SELECT * FROM referee_profiles
                       WHERE last_invited_date >= ?
                       ORDER BY last_invited_date DESC""",
                    (cutoff,),
                ).fetchall()
                return [self._deserialize_profile(r) for r in rows]

    def set_referee_note(self, name: str, note: str) -> bool:
        key = normalize_name(name)
        with self._lock:
            with self._connection() as conn:
                cur = conn.execute(
                    "UPDATE referee_profiles SET notes=? WHERE referee_key=?",
                    (note, key),
                )
                conn.commit()
                return cur.rowcount > 0

    def get_referee_note(self, name: str) -> str | None:
        key = normalize_name(name)
        with self._lock:
            with self._connection() as conn:
                row = conn.execute(
                    "SELECT notes FROM referee_profiles WHERE referee_key=?",
                    (key,),
                ).fetchone()
                return row["notes"] if row else None

    def increment_reminder(self, referee_name: str, journal: str, manuscript_id: str) -> None:
        key = normalize_name(referee_name)
        with self._lock:
            with self._connection() as conn:
                conn.execute(
                    """UPDATE referee_assignments
                       SET reminders_received = reminders_received + 1
                       WHERE referee_key=? AND journal=? AND manuscript_id=?""",
    ...
    def get_top_referees_for_journal(
        self, journal: str, min_invitations: int = 2, limit: int = 10
    ) -> list[dict]:
        with self._lock:
            with self._connection() as conn:
                rows = conn.execute(
                    """SELECT rp.* FROM referee_profiles rp
                       JOIN referee_journal_stats rjs ON rp.referee_key = rjs.referee_key
                       WHERE rjs.journal = ? AND rjs.total_invitations >= ?
                       AND rjs.total_completed > 0
                       ORDER BY rjs.avg_report_quality DESC, rjs.total_completed DESC
                       LIMIT ?""",
                    (journal.lower(), min_invitations, limit),
                ).fetchall()
                return [self._deserialize_profile(r) for r in rows]

    def get_journal_performance(self, journal: str) -> dict | None:
        with self._lock:
            with self._connection() as conn:
                row = conn.execute(
                    """SELECT
                        AVG(days_to_complete) as avg_review_days,
                        AVG(days_to_respond) as avg_response_days,
                        SUM(CASE WHEN response='accepted' THEN 1 ELSE 0 END) * 1.0
                            / MAX(COUNT(*), 1) as acceptance_rate,
                        SUM(was_overdue) * 1.0
                            / MAX(SUM(CASE WHEN returned_date IS NOT NULL THEN 1 ELSE 0 END), 1)
                            as overdue_rate,
                        COUNT(*) as total_assignments
                    FROM referee_assignments WHERE journal = ?""",
                    (journal.lower(),),
                ).fetchone()
    ...
    def store_prediction(
        self, referee_name: str, journal: str, manuscript_id: str, p_accept: float
    ):
        key = normalize_name(referee_name)
        with self._lock:
            with self._connection() as conn:
                conn.execute(
                    """INSERT OR REPLACE INTO model_predictions
                       (referee_key, journal, manuscript_id, predicted_p_accept, predicted_at)
                       VALUES (?, ?, ?, ?, ?)""",
    ...
    def _update_assignment_outcome(
        ...
        with self._lock:
            with self._connection() as conn:
                ...
                conn.execute(
                    """UPDATE referee_assignments
                       SET response = ?, returned_date = ?
                       WHERE referee_key = ? AND journal = ? AND manuscript_id = ?""",
                    (response, returned_date, referee_key, journal.lower(), manuscript_id),
                )
                conn.commit()

    def resolve_predictions(self, journal: str | None = None) -> int:
        with self._lock:
            with self._connection() as conn:
                ...
                for row in rows:
                    asg = conn.execute(
                        """SELECT response, returned_date, days_to_complete
                    ...
                    conn.execute(
                        """UPDATE model_predictions
                           SET actual_accepted = ?, actual_completed = ?,
                               actual_review_days = ?, resolved_at = ?
                           WHERE id = ?""",
                        (
                            actual_accepted,
                            actual_completed,
    ...
    def prediction_calibration(self) -> dict:
        with self._lock:
            with self._connection() as conn:
                rows = conn.execute(
                    """SELECT predicted_p_accept, actual_accepted
                       FROM model_predictions
                       WHERE resolved_at IS NOT NULL
                         AND predicted_p_accept IS NOT NULL
                         AND actual_accepted IS NOT NULL"""
                ).fetchall()
