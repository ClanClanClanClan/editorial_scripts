...
class GmailSearchManager:
    ...
    def initialize(self):
        ...
        token_paths = [
            ...
            Path.home() / "Library/CloudStorage/Dropbox/Work/editorial_scripts/config/gmail_token.json",
    ...
    def search_manuscript_emails(self, manuscript_id: str, referee_emails: List[str], 
        ...
        keyword_query = '(referee OR review OR manuscript OR "peer review" OR revision OR decision OR editor)'
    ...
    def _determine_email_type(self, email_data: Dict) -> str:
        ...
        if any(term in combined_text for term in ['referee report', 'review report', 'reviewer report']):
        ...
        elif any(term in combined_text for term in ['decline', 'unable to review', 'cannot review']):
            return 'referee_declined'
        elif any(term in combined_text for term in ['revision', 'revised manuscript', 'resubmission']):
    ...
    def merge_with_audit_trail(self, audit_trail: List[Dict], external_emails: List[Dict], 
                              manuscript_id: str) -> List[Dict]:
        """
                    ...
                    for event in audit_trail:
                        ...
                        if event_time and isinstance(event_time, datetime):
                            ...
                            if time_diff < 300:  # 5 minutes
                                ...
                                if (email.get('type') == event.get('type') or
                                    self._similar_content(email, event)):
                                    is_duplicate = True
                                    break
        ...
        logger.info(f"Merged timeline: {len(audit_trail)} platform events + "
                   f"{len([e for e in merged_timeline if e.get('external')])} external emails = "
                   f"{len(merged_timeline)} total events")
        
        return merged_timeline
    
    def _create_event_key(self, event: Dict) -> Optional[str]:
...
def enhance_audit_trail_with_gmail(manuscript_data: Dict) -> Dict:
    ...
    try:
        ...
        manuscript_data['external_communications_count'] = len([e for e in complete_timeline if e.get('external')])
        
        logger.info(f"Enhanced timeline for {manuscript_id}: "
