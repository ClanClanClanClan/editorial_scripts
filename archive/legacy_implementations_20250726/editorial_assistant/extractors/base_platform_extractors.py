...
class SIAMExtractor(BaseExtractor, ABC):
    ...
    def _handle_orcid_authentication(self) -> bool:
        ...
        try:
            ...
            sign_in_button = self._wait_for_element_by_xpath(
                "//button[contains(text(), 'Sign in')]", timeout=10
            )
...
class EditorialManagerExtractor(BaseExtractor, ABC):
            ...
            for iframe in iframes:
                try:
                    ...
                    if username_field and password_field:
                        ...
                        if not login_button:
                            login_button = self._wait_for_element_by_xpath(
                                "//input[@value='Editor Login']", timeout=5
                            )
    ...
    def _standard_login(self) -> None:
        ...
        username_selector = platform_config.get("login_selectors", {}).get(
            "username", 'input[id="username"]'
        )
        password_selector = platform_config.get("login_selectors", {}).get(
            "password", 'input[id="password"]'
        )
        submit_selector = platform_config.get("login_selectors", {}).get(
    ...
    def _dismiss_cookie_banner(self) -> None:
        ...
        try:
            # Try by button text
            buttons = self.driver.find_elements("tag name", "button") + self.driver.find_elements(
                "tag name", "input"
            )
            for btn in buttons:
                try:
    ...
    def _is_logged_in(self) -> bool:
        ...
        try:
            ...
            dashboard_indicators = ["Associate Editor", "aries-accordion-item", "editor.main.menu"]
...
class MSPExtractor(BaseExtractor, ABC):
    ...
    def _find_mine_link(self) -> Any | None:
