...
class OnePasswordProvider(CredentialProvider):
    ...
    def _setup_session(self):
        """Set up automatic session management with proper account handling"""
        try:
            # Check for service account token first
            if os.environ.get('OP_SERVICE_ACCOUNT_TOKEN'):
                logger.info("Using 1Password service account token")
                return
            ...
            if result.returncode == 0:
                logger.info("1Password session already active")
                return
                
            # Try to sign in using service account or stored credentials
            logger.info("Setting up 1Password session...")
            
            # Check for service account token
            service_token = os.environ.get('OP_SERVICE_ACCOUNT_TOKEN')
            if service_token:
                logger.info("Using OP_SERVICE_ACCOUNT_TOKEN")
                return
            
            # Try automatic signin for known account
            accounts_result = subprocess.run(['op', 'account', 'list'], 
                                           capture_output=True, text=True, timeout=10)
            if accounts_result.returncode == 0:
                # Sign in to the first available account automatically
                ...
                if signin_result.returncode == 0:
                    ...
                    if 'export OP_SESSION' in signin_result.stdout:
                        session_line = [line for line in signin_result.stdout.split('\n') 
                                      if 'export OP_SESSION' in line]
                        if session_line:
                            session_token = session_line[0].split('=')[1].strip('\'"')
                            os.environ['OP_SESSION_my'] = session_token
                            self.session_token = session_token
                else:
                    logger.warning("Failed to sign in to 1Password automatically")
                    
        except Exception as e:
            logger.warning(f"Could not setup 1Password session: {e}")
        
        # Fallback - try to load existing session
    ...
    def get_credential(self, service: str, field: str = "password") -> Optional[str]:
        ...
        try:
            ...
            # Always specify vault for service accounts
            if self.vault:
                cmd.extend(['--vault', self.vault])
            else:
                # Default vault for service accounts
                cmd.extend(['--vault', 'Private'])
