...
def analyze_extraction_results(manuscripts):
    ...
    total_interests = sum(len(r.get('research_interests', [])) for ms in manuscripts for r in ms.get('referees', []))
    ...
    total_recommended = sum(len(ms.get('referee_recommendations', {}).get('recommended_referees', [])) for ms in manuscripts)
    total_opposed = sum(len(ms.get('referee_recommendations', {}).get('opposed_referees', [])) for ms in manuscripts)
    ...
    print(f"   • Authors with ORCID: {authors_with_orcid}/{total_authors} ({authors_with_orcid/max(total_authors,1)*100:.0f}%)")
    ...
    refs_discovered = sum(1 for ms in manuscripts for r in ms.get('referees', []) if r.get('orcid_discovered'))

    print(f"   • Referees with ORCID: {refs_with_orcid}/{total_referees} ({refs_with_orcid/max(total_referees,1)*100:.0f}%)")
