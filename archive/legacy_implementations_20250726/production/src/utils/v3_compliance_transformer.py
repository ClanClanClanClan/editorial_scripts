...
class V3ComplianceTransformer:
    ...
    def transform_datetime(self, date_str: str) -> str | None:
        ...
        try:
            transformed = {
                ...
                "manuscript_type": self.map_enum_value(
                    manuscript_data.get("article_type", ""), self.MANUSCRIPT_TYPE_MAP, "article"
                ),
                "license_type": self.map_enum_value(
                    manuscript_data.get("license", ""), self.LICENSE_TYPE_MAP, "all-rights-reserved"
                ),
                # Optional fields
    ...
    def transform_authors(self, manuscript_data: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for idx, author_data in enumerate(manuscript_data.get("authors", [])):
            try:
                ...
                authors.append({"author": author, "manuscript_author": manuscript_author})
            ...
            try:
                ...
                review_assignment = {
                    ...
                    "recommendation": self.map_enum_value(
                        referee_data.get("recommendation", ""), self.RECOMMENDATION_MAP
                    ),
                    "report_text": referee_data.get("report", {}).get("content"),
                }

                referees.append({"referee": referee, "review_assignment": review_assignment})

            except Exception as e:
                print(
                    f"      ⚠️ Error transforming referee {referee_data.get('name', 'Unknown')}: {e}"
                )
                continue
    ...
    def _extract_word_count(self, data: dict[str, Any]) -> int | None:
...
def transform_extraction_results(
    input_file: str, output_file: str = None, journal_code: str = "MF"
):
