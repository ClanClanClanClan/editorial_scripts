...
def fix_navigation():
    ...
    try:
        ...
        try:
            device_auth = driver.find_element(
        ...
        if role_elements:
            ...
            for elem in role_elements:
                ...
                if "Associate Editor" in role_text:
                    print("   ✅ Clicking Associate Editor role...")
                    elem.click()
                    time.sleep(3)
                    break
        else:
        ...
        approaches = [
            {
                "name": "Direct URL",
                "action": lambda: driver.get(
                    "https://mc.manuscriptcentral.com/mafi?NEXT_PAGE=ASSOCIATE_EDITOR_MAIN_MENU&ROLE_ID=8194"
                ),
            },
            {
                "name": "Tab navigation",
                "action": lambda: driver.find_element(
                    By.XPATH, "//td[@class='navtab']//a[contains(text(), 'Associate Editor')]"
                ).click(),
            },
            {
                "name": "Menu link",
                "action": lambda: driver.find_element(
                    By.XPATH, "//a[contains(@href, 'ASSOCIATE_EDITOR_MAIN_MENU')]"
                ).click(),
            },
        ]
