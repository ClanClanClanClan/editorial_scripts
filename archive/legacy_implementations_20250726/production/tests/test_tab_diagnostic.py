            ...
            for category in categories:
                if category["count"] > 0:
                    ...
                    if take_action_links:
                        ...
                        tab_images = extractor.driver.find_elements(
                            By.XPATH, "//img[contains(@src, 'lefttabs_')]"
                        )
                        ...
                        try:
                            # Find the image element
                            audit_img = extractor.driver.find_element(
                                By.XPATH, "//img[contains(@src, 'lefttabs_audit_trail')]"
                            )
                            print(
                                f"   Found image: {audit_img.get_attribute('src').split('/')[-1]}"
                            )
