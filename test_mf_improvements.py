...
def test_improvements():
    ...
    try:
        ...
        try:
            ...
            try:
                ae_link = extractor.driver.find_element(By.XPATH, "//a[contains(text(),'Associate Editor')]")
                ae_link.click()
                time.sleep(3)
                print("✅ At AE Center")
            except:
        ...
        category_keywords = [
            'awaiting', 'overdue', 'reviewer', 'scores', 'selection',
            'invitation', 'assignment', 'recommendation', 'decision',
            'revision', 'response', 'with editor', 'under review',
            'in review', 'manuscript', 'submission'
        ]
        ...
        print(f"   Categories with manuscripts: {len([c for c in found_categories if c['count'] > 0])}")
        print(f"   Deduplication working: {'Yes' if test_id in extractor.processed_manuscript_ids else 'No'}")
