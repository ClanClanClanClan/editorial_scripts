...
class ManuscriptPDFExtractor:
    ...
    def __init__(self):
        ...
        self.mf_keywords = {
            'stochastic_processes': ['stochastic', 'brownian', 'markov', 'diffusion', 'jump', 'levy'],
            'optimization': ['optimization', 'optimal', 'control', 'portfolio', 'utility', 'hedging'],
            'derivatives': ['option', 'derivative', 'pricing', 'volatility', 'black-scholes'],
            'risk_management': ['risk', 'var', 'cvar', 'coherent', 'measure', 'capital'],
            'behavioral_finance': ['behavioral', 'sentiment', 'bias', 'psychology', 'irrationality'],
            'market_microstructure': ['microstructure', 'liquidity', 'spread', 'orderbook', 'trading'],
    ...
    def extract_manuscript_content(self, pdf_path: str, manuscript_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"   📊 {content_analysis['word_count']} words, {content_analysis['references_count']} references")
            print(f"   🏷️ {len(content_analysis['keywords'])} keywords, {len(content_analysis['subject_classifications'])} subject areas")
    ...
    def _extract_abstract(self, text: str) -> str:
        ...
        for paragraph in paragraphs[:5]:  # Check first 5 paragraphs
            if 100 <= len(paragraph) <= 2000 and not any(word in paragraph.lower() for word in ['figure', 'table', 'section', 'theorem']):
                return paragraph
    ...
    def _extract_key_terms_tfidf(self, text: str) -> List[str]:
        ...
        try:
            ...
            for term in key_terms:
                if any(mf_term in term.lower() for category in self.mf_keywords.values() for mf_term in category):
    ...
    def _extract_title(self, text: str) -> str:
        ...
        for line in lines[:5]:  # Check first 5 lines
            ...
            if any(skip in line.lower() for skip in ['page', 'doi', 'manuscript', 'received', 'accepted']):
                continue
    ...
    def _analyze_mathematical_content(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'mathematical_sophistication': 'high' if latex_patterns > 20 else 'medium' if latex_patterns > 5 else 'low'
        }
    ...
    def _detect_language_indicators(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'regional_variant': 'us' if us_count > uk_count else 'uk' if uk_count > 0 else 'unknown'
        }
    ...
    def _assess_quality_indicators(self, text: str) -> Dict[str, Any]:
        ...
        technical_terms = len(re.findall(r'(?i)(?:theorem|lemma|proposition|corollary|proof|definition)', text))
