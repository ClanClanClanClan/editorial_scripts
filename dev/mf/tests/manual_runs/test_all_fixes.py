...
try:
    ...
    # Test ORCID search
    test_search = {"name": "Moris Strub", "institution": "Warwick Business School"}
...
try:
    ...
    print(f"\n🆔 ORCID COVERAGE:")
    print(
        f"   AUTHORS: {authors_with_orcid}/{total_authors} ({authors_with_orcid/max(1,total_authors)*100:.0f}%)"
    )
    print(f"      • From journal: {authors_with_orcid - authors_discovered}")
    print(f"      • Discovered via API: {authors_discovered}")
    print(
        f"   REFEREES: {referees_with_orcid}/{total_referees} ({referees_with_orcid/max(1,total_referees)*100:.0f}%)"
    )
    ...
    print(f"\n🌍 COUNTRY COVERAGE:")
    print(
        f"   Authors: {authors_with_country}/{total_authors} ({authors_with_country/max(1,total_authors)*100:.0f}%)"
    )
    print(
        f"   Referees: {referees_with_country}/{total_referees} ({referees_with_country/max(1,total_referees)*100:.0f}%)"
    )
