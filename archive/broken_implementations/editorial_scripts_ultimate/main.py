...
class UltimateSystemManager:
    ...
    async def extract_sicon(self, test_mode: bool = False, headless: bool = True) -> OptimizedExtractionResult:
        ...
        if not self._verify_credentials():
            raise Exception("Missing required credentials. Set ORCID_EMAIL and ORCID_PASSWORD environment variables.")
    ...
    def _save_summary(self, result: OptimizedExtractionResult, summary_file: Path):
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write(f"  👥 Referees: {result.total_referees} ({result.referees_with_emails} with emails)\n")
            ...
            for i, manuscript in enumerate(result.manuscripts, 1):
                ...
                f.write(f"     Authors: {len(manuscript.authors)}, Referees: {len(manuscript.referees)}, PDFs: {len(manuscript.pdf_paths)}\n")
    ...
    def _test_against_baseline(self, result: OptimizedExtractionResult):
        ...
        for criterion, analysis in baseline_test['criteria_analysis'].items():
            ...
            print(f"{status} {criterion.replace('_', ' ').title()}: {analysis['actual']}/{analysis['expected']} ({analysis['percentage']:.0f}%)")
...
async def main():
    ...
    parser.add_argument('journal', choices=['sicon', 'sifin', 'mf', 'mor', 'fs', 'jota'], 
                       help='Journal to extract')
    parser.add_argument('--test', action='store_true', 
                       help='Run in test mode (compare against baseline)')
    ...
    try:
        if args.journal == 'sicon':
            result = await system.extract_sicon(
                test_mode=args.test, 
