...
class DataQualityMetrics:
    ...
    def calculate_success_rates(self) -> dict[str, float]:
        ...
        if self.total_pdfs_attempted > 0:
            rates["pdf_download_rate"] = self.total_pdfs_downloaded / self.total_pdfs_attempted
...
class ExtractionResult:
    ...
    def has_usable_data(self) -> bool:
        ...
        return len(self.manuscripts) > 0 or len(self.referees) > 0 or len(self.pdfs) > 0
    ...
    def calculate_summary_stats(self) -> dict[str, Any]:
        ...
        """
        total_referees = sum(
            len(m.referees) if hasattr(m, "referees") else 0 for m in self.manuscripts
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            "metadata": {
                ...
                if self.metadata and self.metadata.completed_at
                else "",
                "duration_seconds": self.metadata.duration_seconds if self.metadata else 0.0,
            },
            "status": self.status.value,
