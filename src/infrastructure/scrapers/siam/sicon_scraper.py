...
from unified_system.core.base_extractor import Manuscript, Referee
...
class SICONRealExtractor(SIAMBaseExtractor):
    ...
    def __init__(self, cache_dir: Optional[Path] = None, output_dir: Optional[Path] = None):
        ...
        from unified_system.core.smart_cache_manager import SmartCacheManager
        from unified_system.core.enhanced_pdf_manager import EnhancedPDFManager, DocumentStorage
        ...
        self.pdf_manager = EnhancedPDFManager(
            ...
            page=None  # Will be set during extraction
        )
        ...
        try:
            ...
            for href, link_text, count in ae_links:
                try:
                    if int(count) > 0:
                        ...
                        if not await self._goto_with_retry(full_url):
                            logger.warning(f"Failed to navigate to category page: {full_url}")
                            continue
            ...
            for ms_id, url in manuscript_urls.items():
                try:
                    ...
                    if not await self._goto_with_retry(url):
                        logger.warning(f"Failed to navigate to manuscript page: {url}")
                        continue
                    await asyncio.sleep(2)
    ...
    async def _parse_manuscript_page(self, ms_id: str, soup: BeautifulSoup, content: str) -> Optional[Manuscript]:
        ...
        try:
            ...
            logger.debug(f"Parsing manuscript {ms_id} page with {len(soup.find_all('tr'))} table rows")
            
            # Extract fields from table rows - try multiple patterns
            for row in soup.find_all('tr'):
                cells = row.find_all(['td', 'th'])
                if len(cells) >= 2:
                    # Try both td/td and th/td patterns
                    label_cell = cells[0]
                    value_cell = cells[1]
                    ...
                    # Debug log
                    if any(key in label for key in ['Title', 'Author', 'Editor', 'Date']):
                        logger.debug(f"Found field: {label} = {value[:50]}...")
                    
                    # Title matching - be more flexible
                    if any(t in label for t in ['Title', 'Article Title', 'Manuscript Title']):
                        title = value
                        logger.info(f"   📝 Title: {title[:60]}...")
                    
                    # Submission date matching
                    elif any(d in label for d in ['Submission Date', 'Submitted', 'Date Submitted']):
                        submission_date = value
                        logger.info(f"   📅 Submitted: {submission_date}")
                    
                    # Associate Editor matching
                    elif 'Associate Editor' in label or 'AE' in label:
                        associate_editor = value
                        logger.info(f"   👤 AE: {associate_editor}")
                    
                    # Corresponding Editor matching
                    elif 'Corresponding Editor' in label or 'CE' in label:
                        corresponding_editor = value
                        logger.info(f"   👤 CE: {corresponding_editor}")
                    
                    # Author matching - multiple patterns
            ...
            # If we still don't have a title, try to extract from page title or headers
            if not title:
                # Try h1, h2, h3 tags
                for tag in ['h1', 'h2', 'h3']:
                    ...
                    for header in headers:
                        ...
                        if len(header_text) > 20 and ms_id not in header_text:
                            title = header_text
                            logger.info(f"   📝 Title from header: {title[:60]}...")
                            break
                    if title:
                        break
            ...
            manuscript = Manuscript(
                id=ms_id,
                title=title or f"Manuscript {ms_id}",  # Fallback title
                authors=authors or ["Author information not available"],
                status=status,
                submission_date=submission_date,
                journal="SICON",
                corresponding_editor=corresponding_editor,
    ...
    async def _extract_pdfs(self, manuscript: Manuscript):
        """Download all PDFs - simplified approach"""
        try:
