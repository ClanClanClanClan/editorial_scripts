...
from core.models.optimized_models import (
...
class OptimizedPDFDownloader:
    ...
    async def download_pdf(self, url: str, filename: str, output_dir: Path) -> Path | None:
        ...
        try:
            ...
            if not response or response.status != 200:
                logger.error(
                    f"Failed to access PDF URL: {url} (status: {response.status if response else 'No response'})"
                )
                return None
...
class OptimizedCacheManager:
    ...
    async def get_cached_data(self, journal: str, manuscript_id: str) -> dict[str, Any] | None:
...
class OptimizedBaseExtractor(ABC):
    ...
    def __init__(self, output_dir: Path | None = None, config: ConnectionConfig | None = None):
    ...
    async def extract(
        self, headless: bool = True, use_cache: bool = True
    ) -> OptimizedExtractionResult:
        """
        ...
        try:
            ...
            result = OptimizedExtractionResult.create_from_manuscripts(
                self.journal_name, manuscripts
            )
    ...
    async def _handle_cloudflare_challenge(self):
        ...
        if any(
            ...
            for indicator in ["challenge", "Just a moment", "Checking your browser"]
        ):
            logger.warning("Still on CloudFlare page after wait - may need longer wait")
            await asyncio.sleep(30)  # Additional wait

    async def _extract_all_manuscripts_optimized(
        self, use_cache: bool
    ) -> list[OptimizedManuscript]:
            ...
            try:
                ...
                if use_cache:
                    cached_data = await self.cache_manager.get_cached_data(
                        self.journal_name, manuscript_id
                    )
                ...
                if cached_data:
                    ...
                    if manuscript:
                        await self.cache_manager.cache_data(
                            self.journal_name, manuscript_id, manuscript.to_dict()
                        )
    ...
    async def _extract_single_manuscript_optimized(
        self, manuscript_id: str, url: str
    ) -> OptimizedManuscript | None:
