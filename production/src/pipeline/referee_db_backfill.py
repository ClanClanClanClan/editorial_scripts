...
def _auto_feedback(db):
    feedback_count = 0
    for journal in JOURNALS:
        rec_dir = OUTPUTS_DIR / journal / "recommendations"
        if not rec_dir.exists():
            continue
        for rec_path in rec_dir.glob("rec_*.json"):
            try:
                with open(rec_path) as f:
                    rec = json.load(f)
            except (json.JSONDecodeError, OSError):
                continue
            ms_id = rec.get("manuscript_id", "")
            if not ms_id:
                continue
            recommended_names = {
                normalize_name(c.get("name", ""))
                for c in rec.get("referee_candidates", [])
                if c.get("name")
            }
            if not recommended_names:
                continue
            assignments = db.get_assignments_by_manuscript(journal, ms_id)
            if not assignments:
                continue
            assigned_keys = {a["referee_key"] for a in assignments}
            for name_key in recommended_names & assigned_keys:
                db.record_feedback(name_key, journal, ms_id, was_used=True)
                feedback_count += 1
    if feedback_count:
        print(f"   {feedback_count} recommendation(s) confirmed as used")
...
def backfill(incremental: bool = False):
    ...
    print("🔗 Cross-referencing recommendations with assignments...")
    _auto_feedback(db)
    print("✅ Auto-feedback complete")

    print("🔄 Resolving prediction outcomes...")
    resolved = db.resolve_predictions()
