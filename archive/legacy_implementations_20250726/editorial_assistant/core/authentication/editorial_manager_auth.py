...
class EditorialManagerAuth(AuthenticationProvider):
    ...
    def get_login_url(self) -> str:
        ...
        return self.journal_urls.get(
            self.journal_code, f"https://www.editorialmanager.com/{self.journal_code.lower()}/"
        )
        ...
        try:
            ...
            if auth_result.status == AuthStatus.SUCCESS:
                ...
                if self.verify_authentication(driver):
                    self._log_info(
                        f"✅ Editorial Manager authentication successful for {self.journal_code}"
                    )
                    return auth_result
                else:
                    return AuthenticationResult(
    ...
    async def _perform_login(self, driver: WebDriver) -> AuthenticationResult:
        ...
        try:
            ...
            return AuthenticationResult(
                status=AuthStatus.SUCCESS, message="Editorial Manager login successful"
            )
