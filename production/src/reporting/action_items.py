...
def _load_desk_rejection(journal: str, ms_id: str) -> dict | None:
    rec_dir = Path(__file__).resolve().parents[2] / "outputs" / journal.lower() / "recommendations"
    if not rec_dir.exists():
        return None
    matches = sorted(rec_dir.glob(f"rec_{ms_id}_*.json"))
    if not matches:
        return None
    try:
        with open(matches[-1]) as f:
            data = json.load(f)
        return data.get("desk_rejection")
...
def get_seasonal_mode(today=None):
    ...
    for key, period in SEASONAL_SLOWDOWN.items():
        sm, sd, em, ed = (
            period["start_month"],
            period["start_day"],
            period["end_month"],
        ...
        if sm <= em:
            if (today.month, today.day) >= (sm, sd) and (today.month, today.day) <= (em, ed):
                return {"key": key, "label": period["label"]}
        else:
            if (today.month, today.day) >= (sm, sd) or (today.month, today.day) <= (em, ed):
                return {"key": key, "label": period["label"]}
    return None
...
def compute_action_items(journals: list[str] | None = None) -> list[RefereeAction]:
    ...
    for journal in target:
        ...
        for ms in data.get("manuscripts", []):
            ...
            ms_status_lower = ms_status.lower()
            is_revision = any(k in ms_status_lower for k in ("r1", "r2", "r3", "revision")) or bool(
                re.search(r"-R\d+", ms_id)
            )
            ...
            for ref in referees:
                ...
                if norm == "agreed" and due:
                    ...
                    if days_left < -extra_days:
                        overdue_priority = "high" if seasonal else "critical"
                        items.append(
                            RefereeAction(
                        ...
                        due_priority = "high" if is_revision and days_left <= 7 else "medium"
                        due_msg = f"{ref_name}'s report due in {days_left} days"
                        if is_revision:
                            due_msg += " (revision)"
                        items.append(
                            RefereeAction(
            ...
            desk = _load_desk_rejection(journal, ms_id)
            if desk and desk.get("should_desk_reject") and desk.get("confidence", 0) > 0.5:
                items.append(
                    RefereeAction(
                        priority="high",
                        ...
                        manuscript_title=ms_title,
                        status=ms_status,
                        message=f"Desk rejection recommended (confidence={desk['confidence']:.0%}): {desk.get('summary', '')}",
                    )
                )
