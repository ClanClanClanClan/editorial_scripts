"""JOTA journal adapter (Springer platform) – scaffold implementation."""
...
class JOTAAdapter(AsyncJournalAdapter):
    ...
    async def fetch_manuscripts(self, categories: list[str]) -> list[Manuscript]:
        try:
            ...
            from src.platforms.springer_parsers import parse_list_html

            items = parse_list_html(html, r"JOTA-\d{4}-\d{4}")
            return [
                Manuscript(
        ...
        try:
            if hasattr(self.page, "query_selector"):
                try:
                    ...
                    pass
            html = await self.page.content()
            from src.platforms.springer_parsers import (
            ...
            from src.ecc.core.domain.models import Author, DocumentType, File, Referee

            for name in details.get("authors", []):
                ms.authors.append(Author(name=name))
            for r in refs:
                ref = Referee(name=r.get("name", ""))
                ref.historical_performance = {"status": r.get("status", "")}
                ms.referees.append(ref)
            if events:
                ms.metadata["audit_trail"] = normalize_events(events)
            # Files: add placeholders now and keep links for later downloads
