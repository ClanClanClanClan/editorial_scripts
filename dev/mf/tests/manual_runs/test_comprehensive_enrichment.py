...
def analyze_extraction_completeness(results):
    ...
    for ms in results:
        ...
        if (
            ...
            and funding != "There are no funders to report for this submission"
        ):
        ...
        if rec_refs:
            ...
            if rec_count > 0 or opp_count > 0:
                ...
                for rec in rec_refs.get("recommended_referees", [])[:2]:
                    print(
                        f"   • Recommended: {rec.get('name', 'Unknown')} ({rec.get('institution', 'N/A')})"
                    )
                for opp in rec_refs.get("opposed_referees", [])[:2]:
                    print(
                        f"   • Opposed: {opp.get('name', 'Unknown')} ({opp.get('institution', 'N/A')})"
                    )
            else:
        ...
        for i, referee in enumerate(referees, 1):
            ...
            print(
                f"      • Institution: {referee.get('institution_parsed', referee.get('affiliation', 'Not found'))}"
            )
    ...
    authors_with_orcid = sum(1 for ms in results for a in ms.get("authors", []) if a.get("orcid"))
    referees_with_orcid = sum(1 for ms in results for r in ms.get("referees", []) if r.get("orcid"))

    # Count discovered ORCIDs
    authors_discovered = sum(
        1 for ms in results for a in ms.get("authors", []) if a.get("orcid_discovered")
    )
    referees_discovered = sum(
        1 for ms in results for r in ms.get("referees", []) if r.get("orcid_discovered")
    )

    # Count countries
    authors_with_country = sum(
        1 for ms in results for a in ms.get("authors", []) if a.get("country")
    )
    referees_with_country = sum(
        1 for ms in results for r in ms.get("referees", []) if r.get("country")
    )
    ...
    print(f"\n🆔 ORCID COVERAGE:")
    print(
        f"   • Authors with ORCID: {authors_with_orcid}/{total_authors} ({authors_with_orcid/total_authors*100:.0f}%)"
    )
    print(f"     - From journal: {authors_with_orcid - authors_discovered}")
    print(f"     - Discovered via API: {authors_discovered}")
    print(
        f"   • Referees with ORCID: {referees_with_orcid}/{total_referees} ({referees_with_orcid/total_referees*100:.0f}%)"
    )
    print(f"     - From journal: {referees_with_orcid - referees_discovered}")
    print(f"     - Discovered via API: {referees_discovered}")
    print(f"\n🌍 COUNTRY COVERAGE:")
    print(
        f"   • Authors with country: {authors_with_country}/{total_authors} ({authors_with_country/total_authors*100:.0f}%)"
    )
    print(
        f"   • Referees with country: {referees_with_country}/{total_referees} ({referees_with_country/total_referees*100:.0f}%)"
    )
    ...
    ms_with_funding = sum(
        1
        for ms in results
        if ms.get("funding_information")
        and ms["funding_information"]
        not in ["Not specified", "There are no funders to report for this submission"]
    )
    print(f"\n💰 FUNDING:")
