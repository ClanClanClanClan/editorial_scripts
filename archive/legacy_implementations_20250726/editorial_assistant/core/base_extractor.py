...
from .exceptions import CheckpointError, ExtractionError
...
class BaseExtractor(ABC):
    ...
    def __init__(self, journal: Journal, headless: bool = True, checkpoint_dir: Path | None = None):
        """
    ...
    def _save_checkpoint(self) -> None:
        ...
        checkpoint_file = (
            self.checkpoint_dir / f"checkpoint_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def _get_referee_status_breakdown(self) -> dict[str, int]:
        ...
        for manuscript in self.manuscripts:
            for referee in manuscript.referees:
                status = (
                    referee.status.value
                    if isinstance(referee.status, RefereeStatus)
                    else str(referee.status)
                )
    ...
    def _get_manuscript_status_breakdown(self) -> dict[str, int]:
        ...
        for manuscript in self.manuscripts:
            status = (
                ...
                if isinstance(manuscript.status, ManuscriptStatus)
                else str(manuscript.status)
            )
            breakdown[status] = breakdown.get(status, 0) + 1
        return breakdown
    ...
    def safe_find_element(self, selector: str, by: By = By.CSS_SELECTOR) -> Any | None:
        """
