...
from .commands import analyze, extract, report
...
def cli(ctx, debug, config_dir):
    ...
    ctx.obj = {"config_loader": config_loader, "console": console}
...
def init(force):
    ...
    for filename, source in files.items():
        ...
        if target.exists() and not force:
            console.print(
                f"[yellow]⚠️  {filename} already exists (use --force to overwrite)[/yellow]"
            )
            continue
        ...
        if filename == "credentials.yaml":
            ...
            if source_path.exists():
                ...
                target.write_text(
                    """# Editorial Assistant Credentials
