...
class PDFHandler:
    ...
    def download_pdf_from_url(
        self, url: str, filename: str, cookies: dict[str, str] | None = None
    ) -> Path | None:
        """
    ...
    def extract_pdf_from_page(self, driver: WebDriver, manuscript_id: str) -> Path | None:
        """
            ...
            try:
                ...
                if pdf_element:
                    ...
                    if href and self._is_pdf_url(href):
                        ...
                        return self.download_pdf_from_url(
                            href, f"{manuscript_id}_manuscript.pdf", cookies
                        )
                    else:
                        ...
                        if self._is_pdf_url(driver.current_url):
                            return self._download_pdf_from_current_page(
                                driver, f"{manuscript_id}_manuscript.pdf"
                            )

                        # Otherwise check downloads folder
                        return self.download_pdf_from_browser(
                            driver,
                            Path(driver.capabilities.get("chrome", {}).get("userDataDir", "."))
                            / "Downloads",
    ...
    def _download_pdf_from_current_page(self, driver: WebDriver, filename: str) -> Path | None:
