    ...
    async def initialize(self) -> None:
        ...
        async with self._lock:
            try:
                ...
                self.logger.info(f"Browser pool initialized successfully with {len(self.active_sessions)} sessions")
...
@asynccontextmanager
async def browser_pool(pool_size: int = 3, config: Optional[BrowserConfig] = None,
                      logger: Optional[logging.Logger] = None):
    """
