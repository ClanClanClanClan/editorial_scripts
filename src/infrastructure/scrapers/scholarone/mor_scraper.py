"""
Mathematics of Operations Research (MOR) Journal Scraper - Fixed Version
...
class MORScraperFixed(BaseScraper):
    """Mathematics of Operations Research journal scraper for ScholarOne platform"""
    
    def __init__(self):
        """Initialize MOR scraper"""
        super().__init__(
            name="MOR_Scraper",
            base_url="https://mc.manuscriptcentral.com/mathor",
    ...
    async def authenticate(self, page: Page) -> bool:
        ...
        try:
            self.logger.info("🔐 Starting MOR authentication...")
            ...
            username = (
                os.environ.get('MOR_USER') or 
                settings.mor_username or
                os.environ.get('MF_USER') or 
                settings.mf_username or
                os.environ.get('SCHOLARONE_USER') or 
                settings.scholarone_username
            )
            
            password = (
                os.environ.get('MOR_PASS') or 
                settings.mor_password or
                os.environ.get('MF_PASS') or 
                settings.mf_password or
                os.environ.get('SCHOLARONE_PASS') or 
                settings.scholarone_password
            )
            
            if not username or not password:
                raise ValueError("MOR/MF/ScholarOne credentials not configured")
            ...
            if "Dashboard" in current_url or "Author" in current_url:
                ...
                self.logger.info("✅ MOR authentication successful")
                return True
            else:
                self.logger.error("❌ MOR authentication failed")
                return False
    ...
    async def extract_manuscripts(self, page: Page) -> List[Manuscript]:
        """Extract manuscripts from MOR dashboard"""
        try:
    ...
    async def _download_manuscript_documents(self, ms_data: Dict[str, Any]):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if ms_data['documents'].get('referee_reports'):
                ms_data['documents']['referee_reports_local'] = []
                for i, report_url in enumerate(ms_data['documents']['referee_reports']):
                    try:
                        report_path = await self._download_document(
                            session,
                            report_url,
                            manuscript_id,
                            f'referee_report_{i+1}.pdf'
                        )
                        ms_data['documents']['referee_reports_local'].append(report_path)
                        self.logger.info(f"✅ Downloaded referee report {i+1} for {manuscript_id}")
                    except Exception as e:
                        self.logger.error(f"Failed to download referee report {i+1}: {e}")
